...
cpdef e( object hum_dir, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm1n8 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm1n8
    ores = onumm1n8_create_r(0.0)

    # Set the coefficient to 1.
    onumm1n8_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm1n8.create(&ores)

  else:
    
    fres = feonumm1n8(0.0, nip)
    feonumm1n8_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n8
    ores = onumm1n8_create_r(1.0)
    res = onumm1n8.create(&ores)

  else:
    
    fres = feonumm1n8(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n8
    ores = onumm1n8_create_r(num)
    res = onumm1n8.create(&ores)

  else:
    
    fres = feonumm1n8(num, nip)
    res = fres
...
cpdef eye(uint64_t size, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n8.
    Ores = oarrm1n8_eye(size)

    return omatm1n8.create(&Ores)

  else:

    # This will create a feomatm1n8
    Fres = feoarrm1n8_eye(size,nip)

    return feomatm1n8.create(&Fres)
...
cpdef array( object arr,  uint64_t nip = 0):
  ...
  cdef oarrm1n8_t   Oarr
  cdef feoarrm1n8_t Farr
  cdef omatm1n8     Ores
  cdef feomatm1n8   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm1n8) or (tin is feomatm1n8):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm1n8_zeros( nrows, ncols)
        Ores = omatm1n8.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm1n8_zeros( nrows, ncols, nip)
        Fres = feomatm1n8.create(&Farr)
...
cpdef zeros( object shape_in, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n8.
    Ores = oarrm1n8_zeros(nrows,ncols)
    ...
    # This will create a feomatm1n8
    Fres = feoarrm1n8_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n8.
    Ores = oarrm1n8_ones(nrows,ncols)
    ...
    # This will create a feomatm1n8
    Fres = feoarrm1n8_ones(nrows,ncols,nip)
           # feoarrm1n8_ones(nrows,ncols,nip,nbases,order)
