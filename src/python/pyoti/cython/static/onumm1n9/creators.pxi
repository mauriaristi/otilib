...
cpdef e( object hum_dir, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm1n9 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm1n9
    ores = onumm1n9_create_r(0.0)

    # Set the coefficient to 1.
    onumm1n9_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm1n9.create(&ores)

  else:
    
    fres = feonumm1n9(0.0, nip)
    feonumm1n9_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n9
    ores = onumm1n9_create_r(1.0)
    res = onumm1n9.create(&ores)

  else:
    
    fres = feonumm1n9(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n9
    ores = onumm1n9_create_r(num)
    res = onumm1n9.create(&ores)

  else:
    
    fres = feonumm1n9(num, nip)
    res = fres
...
cpdef eye(uint64_t size, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n9.
    Ores = oarrm1n9_eye(size)

    return omatm1n9.create(&Ores)

  else:

    # This will create a feomatm1n9
    Fres = feoarrm1n9_eye(size,nip)

    return feomatm1n9.create(&Fres)
...
cpdef array( object arr,  uint64_t nip = 0):
  ...
  cdef oarrm1n9_t   Oarr
  cdef feoarrm1n9_t Farr
  cdef omatm1n9     Ores
  cdef feomatm1n9   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm1n9) or (tin is feomatm1n9):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm1n9_zeros( nrows, ncols)
        Ores = omatm1n9.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm1n9_zeros( nrows, ncols, nip)
        Fres = feomatm1n9.create(&Farr)
...
cpdef zeros( object shape_in, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n9.
    Ores = oarrm1n9_zeros(nrows,ncols)
    ...
    # This will create a feomatm1n9
    Fres = feoarrm1n9_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n9.
    Ores = oarrm1n9_ones(nrows,ncols)
    ...
    # This will create a feomatm1n9
    Fres = feoarrm1n9_ones(nrows,ncols,nip)
           # feoarrm1n9_ones(nrows,ncols,nip,nbases,order)
