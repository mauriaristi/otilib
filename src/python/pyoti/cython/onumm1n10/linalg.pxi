...
cpdef dot(object lhs, object rhs, object out = None):
  ...
  elif tlhs is csr_omatm1n10:

    # Slhs = lhs
    if   trhs is omatm1n10: # SO
      if res_flag:
        Ores = out
        csromatm1n10_matmul_SO_to( lhs, rhs, Ores)
      else:
        res = csromatm1n10_matmul_SO( lhs, rhs)
      # end if 
...
cpdef solve(object K_in, omatm1n10 b_in, omatm1n10 out = None):
  ...
  cdef csr_omatm1n10  S, Sres, Stmp  
  cdef uint64_t i,j,k,l
  cdef ord_t ordi, ord_lhs, ord_rhs, Oord
  cdef uint8_t res_flag = 1
  cdef object res
...
#*****************************************************************************************************
cdef solve_sparse(csr_omatm1n10 K_in, omatm1n10 b_in, omatm1n10 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
cpdef set_order_im_from_array(ord_t ordi, np.ndarray arr, omatm1n10 tmp):
  ...
  #     tmp[i,j] += onumm1n10.create( &otmp, FLAGS = 0)
