...
cpdef e( object hum_dir, bases_t nbases = 0 , ord_t order = 0, uint64_t nip = 0):
  ...
  INPUTS:
  - hum_dir = Object that defines the imaginary direction in a human-friendly manner.
  - nbases  = Optional. Defines the number of imaignary bases of the number. If not set, it will 
              only allocate memory for a single imaginary direction at the specified order. 
              (Default bases_t nbases = 0 )
  - order   = Optional. Defines the truncation order of the number. The truncation order is defined 
              as the maximum between the order of the given imaginary direction and the order given
              in this parameter.  (Default ord_t order = 0)
  - nip     = Defines the number of integration points to create this number. This is useful when
...
#*****************************************************************************************************
cpdef deriv( object hum_dir, bases_t nbases = 0 , ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a sotinum from a specified derivative. It computes the propper factor and 
            sets the coefficient in the corresponding imaginary direction.
  ...
  if nip == 0: 
    ...
    # Set the coefficient to 1.
    soti_set_deriv_r(1.0, indx_hd, order_hd, &ores, dhl) 
    ...
    fesoti_set_deriv_r( 1.0, indx_hd, order_hd, &fres.num, dhl)
