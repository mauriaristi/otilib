...
cdef class mdmatfe3:
    ...
    for i in range(self.arr.nrows):
      for j in range(self.arr.ncols):
        for k in range(self.arr.nip):
          ...
          fedarr_set_item_ijk_r( mdnum3_tmp.r , i, j, k, &tmp.arr);
        # end for
      # end for
    # end for
  ...
  cpdef set_ijk(self, object rhs, uint64_t i, uint64_t j, uint64_t k):
    ...
    if   trhs is mdnum3:
      ...
      orhs = rhs
      femdarr3_set_item_ijk_o( &orhs.num, i, j, k, &self.arr)
      ...
      try:
        ...
        rrhs = rhs
        femdarr3_set_item_ijk_r( rrhs, i, j, k, &self.arr)
      
      except:
      
        raise ValueError("Supported values are real scalar and mdnum3.")
