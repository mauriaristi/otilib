...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm3n2 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm3n2
    ores = onumm3n2_create_r(0.0)

    # Set the coefficient to 1.
    onumm3n2_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm3n2.create(&ores)

  else:
    
    fres = feonumm3n2(0.0, nip)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm3n2
    ores = onumm3n2_create_r(1.0)
    res = onumm3n2.create(&ores)

  else:
    
    fres = feonumm3n2(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm3n2
    ores = onumm3n2_create_r(num)
    res = onumm3n2.create(&ores)

  else:
    
    fres = feonumm3n2(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm3n2.
    Ores = oarrm3n2_eye(size)

    return omatm3n2.create(&Ores)

  else:

    # This will create a feomatm3n2
    Fres = feoarrm3n2_eye(size,nip)

    return feomatm3n2.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  cdef omatm3n2     Ores
  cdef feomatm3n2   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm3n2) or (tin is feomatm3n2):
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm3n2.
    Ores = oarrm3n2_zeros(nrows,ncols)
    ...
    # This will create a feomatm3n2
    Fres = feoarrm3n2_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm3n2.
    Ores = oarrm3n2_ones(nrows,ncols)
    ...
    # This will create a feomatm3n2
    Fres = feoarrm3n2_ones(nrows,ncols,nip)
