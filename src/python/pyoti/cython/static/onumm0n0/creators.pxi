...
cpdef e( object hum_dir, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm0n0 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm0n0
    ores = onumm0n0_create_r(0.0)

    # Set the coefficient to 1.
    onumm0n0_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm0n0.create(&ores)

  else:
    
    fres = feonumm0n0(0.0, nip)
    feonumm0n0_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm0n0
    ores = onumm0n0_create_r(1.0)
    res = onumm0n0.create(&ores)

  else:
    
    fres = feonumm0n0(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm0n0
    ores = onumm0n0_create_r(num)
    res = onumm0n0.create(&ores)

  else:
    
    fres = feonumm0n0(num, nip)
    res = fres
...
cpdef eye(uint64_t size, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm0n0.
    Ores = oarrm0n0_eye(size)

    return omatm0n0.create(&Ores)

  else:

    # This will create a feomatm0n0
    Fres = feoarrm0n0_eye(size,nip)

    return feomatm0n0.create(&Fres)
...
cpdef array( object arr,  uint64_t nip = 0):
  ...
  cdef oarrm0n0_t   Oarr
  cdef feoarrm0n0_t Farr
  cdef omatm0n0     Ores
  cdef feomatm0n0   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm0n0) or (tin is feomatm0n0):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm0n0_zeros( nrows, ncols)
        Ores = omatm0n0.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm0n0_zeros( nrows, ncols, nip)
        Fres = feomatm0n0.create(&Farr)
...
cpdef zeros( object shape_in, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm0n0.
    Ores = oarrm0n0_zeros(nrows,ncols)
    ...
    # This will create a feomatm0n0
    Fres = feoarrm0n0_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm0n0.
    Ores = oarrm0n0_ones(nrows,ncols)
    ...
    # This will create a feomatm0n0
    Fres = feoarrm0n0_ones(nrows,ncols,nip)
           # feoarrm0n0_ones(nrows,ncols,nip,nbases,order)
