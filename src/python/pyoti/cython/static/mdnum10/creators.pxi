...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a mdnum10 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a mdnum10
    ores = mdnum10_create_r(0.0)

    # Set the coefficient to 1.
    mdnum10_set_item(1.0, indx_hd, order_hd, &ores) 
    res = mdnum10.create(&ores)

  else:
    
    fres = mdnumfe10(0.0, nip)
    femdnum10_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum10
    ores = mdnum10_create_r(1.0)
    res = mdnum10.create(&ores)

  else:
    
    fres = mdnumfe10(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum10
    ores = mdnum10_create_r(num)
    res = mdnum10.create(&ores)

  else:
    
    fres = mdnumfe10(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an mdmat10.
    Ores = mdarr10_eye(size)

    return mdmat10.create(&Ores)

  else:

    # This will create a mdmatfe10
    Fres = femdarr10_eye(size,nip)

    return mdmatfe10.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if (tin is mdmat10) or (tin is mdmatfe10):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = mdarr10_zeros( nrows, ncols)
        Ores = mdmat10.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = femdarr10_zeros( nrows, ncols, nip)
        Fres = mdmatfe10.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an mdmat10.
    Ores = mdarr10_zeros(nrows,ncols)
    ...
    # This will create a mdmatfe10
    Fres = femdarr10_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create a mdmatfe10
    Fres = femdarr10_ones(nrows,ncols,nip)
           # femdarr10_ones(nrows,ncols,nip,nbases,order)
