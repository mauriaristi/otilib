...
cpdef e( object hum_dir, uint64_t nip = 0):
  """
  PURPOSE:  To create a mdnum2 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a mdnum2
    ores = mdnum2_create_r(0.0)

    # Set the coefficient to 1.
    mdnum2_set_item(1.0, indx_hd, order_hd, &ores) 
    res = mdnum2.create(&ores)

  else:
    
    fres = mdnumfe2(0.0, nip)
...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum2
    ores = mdnum2_create_r(1.0)
    res = mdnum2.create(&ores)

  else:
    
    fres = mdnumfe2(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum2
    ores = mdnum2_create_r(num)
    res = mdnum2.create(&ores)

  else:
    
    fres = mdnumfe2(num, nip)
    res = fres
...
cpdef eye(uint64_t size, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an mdmat2.
    Ores = mdarr2_eye(size)

    return mdmat2.create(&Ores)

  else:

    # This will create a mdmatfe2
    Fres = femdarr2_eye(size,nip)

    return mdmatfe2.create(&Fres)
...
cpdef array( object arr,  uint64_t nip = 0):
  ...
  if (tin is mdmat2) or (tin is mdmatfe2):
...
cpdef zeros( object shape_in, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create a mdmatfe2
    Fres = femdarr2_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create a mdmatfe2
    Fres = femdarr2_ones(nrows,ncols,nip)
