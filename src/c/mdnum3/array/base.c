...
void mdarr3_taylor_integrate_to( coeff_t* deltas, mdarr3_t* arr, mdarr3_t* res){
    ...
    mdarr3_dimCheck_OO_elementwise(arr,res,res);

    for (i = 0; i<arr->size; i++ ){
        
        mdnum3_taylor_integrate_to( deltas, &arr->p_data[i], &res->p_data[i]);
...
void mdarr3_get_order_im_to( ord_t order, mdarr3_t* arr, mdarr3_t* res){
    ...
    for (i = 0; i<arr->size; i++ ){
        ...
        mdnum3_get_order_im_to( order, &arr->p_data[i], &res->p_data[i]);
...
void mdarr3_truncate_im_to( imdir_t idx, ord_t order, mdarr3_t* arr, mdarr3_t* res){
    ...
    for (i = 0; i<arr->size; i++ ){
        ...
        mdnum3_truncate_im_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
void mdarr3_copy_to(mdarr3_t* arr, mdarr3_t* res){
    ...
    for (i=0; i<arr->size; i++){
        ...
        mdnum3_copy_to(&arr->p_data[i],&res->p_data[i]);
...
// ****************************************************************************************************
inline void mdarr3_set_all_r( coeff_t num, mdarr3_t* arr){
    
    uint64_t i;

    for (i = 0; i<arr->size; i++ ){
        mdarr3_set_item_i_r( num, i, arr);
    }
...
// ****************************************************************************************************
inline void mdarr3_set_r( coeff_t num, mdarr3_t* arr){
...
// ****************************************************************************************************
inline void mdarr3_set_item_i_r( coeff_t num, uint64_t i, mdarr3_t* arr){
        
    if( i<arr->size ){
        
        mdnum3_set_r( num, &arr->p_data[i]);

    } else {

        // res = mdnum3_init();
        
        printf("ERROR: Index out of bounds in mdarr3_get_item_i(...)\n Exiting...\n");
        exit(OTI_BadIndx);
...
// ****************************************************************************************************
inline void mdarr3_set_item_ij_r( coeff_t num, uint64_t i, uint64_t j, mdarr3_t* arr){

    mdarr3_set_item_i_r( num, j + i*arr->ncols, arr);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
inline void mdarr3_set_all_o( mdnum3_t* num, mdarr3_t* arr){

    uint64_t i;

    for (i = 0; i<arr->size; i++ ){
        mdarr3_set_item_i_o( num, i, arr);
    }
...
// ****************************************************************************************************
inline void mdarr3_set_o( mdnum3_t* num, mdarr3_t* arr){
...
// ****************************************************************************************************
inline void mdarr3_set_item_i_o( mdnum3_t* src, uint64_t i, mdarr3_t* arr){
    
    if (i < arr->size){

        mdnum3_copy_to(src, &arr->p_data[i]);
...
// ****************************************************************************************************
inline void mdarr3_set_item_ij_o( mdnum3_t* num, uint64_t i, uint64_t j, mdarr3_t* arr){

    if ( i < arr->nrows && j < arr->ncols ){

        mdarr3_set_item_i_o( num, j + i*arr->ncols, arr);

    } else {

        printf("ERROR: Index out of bounds in mdarr3_get_item_ij(...)\n Exiting...\n");
        exit(OTI_BadIndx);
...
// ****************************************************************************************************
void mdarr3_set_slice_O( mdarr3_t* arr, 
...
// ****************************************************************************************************
void mdarr3_set_slice_o( mdnum3_t* num, 
                        int64_t starti, int64_t stopi, int64_t stepi,
                        int64_t startj, int64_t stopj, int64_t stepj,
                         mdarr3_t* res){
...
darr_t mdarr3_get_im(imdir_t idx, ord_t order, mdarr3_t* arr){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        darr_set_item_i(  
            mdnum3_get_item( idx, order, &arr->p_data[i]), 
            i, &res);
...
void mdarr3_extract_im_to(imdir_t idx, ord_t order, mdarr3_t* arr, mdarr3_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum3_extract_im_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
darr_t mdarr3_get_deriv(imdir_t idx, ord_t order, mdarr3_t* arr){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        darr_set_item_i(  
            mdnum3_get_deriv( idx, order, &arr->p_data[i]), 
            i, &res);
...
void mdarr3_get_im_to(imdir_t idx, ord_t order, mdarr3_t* arr, mdarr3_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum3_get_im_to_o( idx, order, &arr->p_data[i], &res->p_data[i]);
...
void mdarr3_get_deriv_to(imdir_t idx, ord_t order, mdarr3_t* arr, mdarr3_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum3_get_deriv_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
void mdarr3_extract_deriv_to(imdir_t idx, ord_t order, mdarr3_t* arr, mdarr3_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum3_extract_deriv_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
// Getters.
// ****************************************************************************************************
inline mdnum3_t mdarr3_get_item_i(mdarr3_t* arr, uint64_t i){
...
mdnum3_t mdarr3_get_item_ij(mdarr3_t* arr, uint64_t i, uint64_t j){
    ...
    mdnum3_t res = mdarr3_get_item_i(arr, j + i*arr->ncols );
...
void mdarr3_get_item_i_to(mdarr3_t* arr, uint64_t i, mdnum3_t* res){
    ...
    if (i < arr->size){
        ...
        mdnum3_copy_to( &arr->p_data[i], res);
        
    } else {
        // Raise error?
        printf("ERROR: Index out of bounds in mdarr3_get_item_i_to(...)\n");
        exit(OTI_BadIndx);
    }
...
void mdarr3_get_item_ij_to(mdarr3_t* arr, uint64_t i, uint64_t j, mdnum3_t* res){
    ...
    mdarr3_get_item_i_to(arr, j + i*arr->ncols, res);
...
// ****************************************************************************************************
inline mdarr3_t mdarr3_createEmpty(uint64_t nrows, uint64_t ncols){

    mdarr3_t  res ;
    uint64_t size = nrows * ncols;

    res.p_data = (mdnum3_t*) malloc( size * sizeof( mdnum3_t ) );

    if ( res.p_data == NULL ){
        printf("ERROR: Not enough memory to handle mdarr3.\n Exiting...\n");
...
// Initialization
// ****************************************************************************************************
inline mdarr3_t mdarr3_init(void){
