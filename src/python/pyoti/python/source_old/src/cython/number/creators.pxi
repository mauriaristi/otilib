...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a {num_pytype}
    ores = {num_func}_create_r(1.0, nbases, order)
    res = {num_pytype}.create(&ores)
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a {num_pytype}
    ores = {num_func}_create_r(num, nbases, order)
    res = {num_pytype}.create(&ores)
  ...
  if (tin is {arr_pytype}) or (tin is {fearr_pytype}):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = {arr_func}_zeros( nrows, ncols, nbases, order)
        ...
        Farr = {fearr_func}_zeros( nrows, ncols, nip, nbases, order)
