...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a mdnum7 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a mdnum7
    ores = mdnum7_create_r(0.0)

    # Set the coefficient to 1.
    mdnum7_set_item(1.0, indx_hd, order_hd, &ores) 
    res = mdnum7.create(&ores)

  else:
    
    fres = mdnumfe7(0.0, nip)
    femdnum7_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum7
    ores = mdnum7_create_r(1.0)
    res = mdnum7.create(&ores)

  else:
    
    fres = mdnumfe7(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum7
    ores = mdnum7_create_r(num)
    res = mdnum7.create(&ores)

  else:
    
    fres = mdnumfe7(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an mdmat7.
    Ores = mdarr7_eye(size)

    return mdmat7.create(&Ores)

  else:

    # This will create a mdmatfe7
    Fres = femdarr7_eye(size,nip)

    return mdmatfe7.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if (tin is mdmat7) or (tin is mdmatfe7):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = mdarr7_zeros( nrows, ncols)
        Ores = mdmat7.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = femdarr7_zeros( nrows, ncols, nip)
        Fres = mdmatfe7.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create a mdmatfe7
    Fres = femdarr7_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create a mdmatfe7
    Fres = femdarr7_ones(nrows,ncols,nip)
           # femdarr7_ones(nrows,ncols,nip,nbases,order)
