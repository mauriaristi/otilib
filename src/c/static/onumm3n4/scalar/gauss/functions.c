

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_atanh(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);

    feonumm3n4_atanh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_asinh(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);

    feonumm3n4_asinh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_acosh(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);

    feonumm3n4_acosh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_tanh(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);

    feonumm3n4_tanh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_cosh(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_cosh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_sinh(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_sinh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_asin(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_asin_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_acos(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_acos_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_atan(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_atan_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_tan(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_tan_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_cos(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_cos_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_sin(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_sin_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_logb(feonumm3n4_t* num, double base){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_logb_to(num, base, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_log10(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_log10_to(num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_log(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_log_to(num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_exp(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_exp_to(num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_cbrt(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);

    feonumm3n4_cbrt_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_sqrt(feonumm3n4_t* num){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);
    
    feonumm3n4_sqrt_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm3n4_t feonumm3n4_pow(feonumm3n4_t* num, double e){

    feonumm3n4_t res = feonumm3n4_init();

    res = feonumm3n4_createEmpty(num->nip);

    feonumm3n4_pow_to(num, e, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_atanh_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_atanh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_asinh_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_asinh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_acosh_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_acosh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_tanh_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_tanh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_cosh_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_cosh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_sinh_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_sinh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_asin_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_asin_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_acos_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_acos_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_atan_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_atan_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_tan_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_tan_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_cos_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_cos_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_sin_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_sin_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_logb_to(feonumm3n4_t* num, double base, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_logb_to(&num->p_data[i], base, &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_log10_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_log10_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_log_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_log_to(&num->p_data[i], &res->p_data[i]);

    } 

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_exp_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_exp_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_cbrt_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_cbrt_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_sqrt_to(feonumm3n4_t* num, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_sqrt_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_pow_to(feonumm3n4_t* num, double e, feonumm3n4_t* res){

    uint64_t i;

    feonumm3n4_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm3n4_pow_to(&num->p_data[i], e, &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm3n4_t feonumm3n4_integrate_r(coeff_t num, feonumm3n4_t* w){

    onumm3n4_t  res = onumm3n4_init(); // res = 0.0

    feonumm3n4_integrate_r_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_integrate_r_to(coeff_t num, feonumm3n4_t* w, onumm3n4_t* res){

    uint64_t i = 0;

    onumm3n4_t tmp ;

    // res may not come as zero, Modify this?
    onumm3n4_set_r(0,&tmp);

    // The loop for every element in num.
    for (i = 0; i < w->nip; i++){
        
        onumm3n4_sum_oo_to(  &tmp, &w->p_data[i], &tmp);

    }

    onumm3n4_mul_ro_to( num, &tmp, res);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm3n4_t feonumm3n4_integrate_o(onumm3n4_t* num, feonumm3n4_t* w){

    onumm3n4_t  res = onumm3n4_init(); // res = 0.0

    feonumm3n4_integrate_o_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_integrate_o_to(onumm3n4_t* num, feonumm3n4_t* w, onumm3n4_t* res){

    uint64_t i = 0;

    onumm3n4_t tmp ;

    // res may not come as zero, Modify this?
    onumm3n4_set_r(0, &tmp);

    // The loop for every element in num.
    for (i = 0; i < w->nip; i++){
        
        onumm3n4_sum_oo_to(  &tmp, &w->p_data[i], &tmp);

    }

    onumm3n4_mul_oo_to( num, &tmp, res);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm3n4_t feonumm3n4_integrate_f(feonumm3n4_t* num, feonumm3n4_t* w){

    onumm3n4_t  res = onumm3n4_init(); // res = 0.0

    feonumm3n4_integrate_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_integrate_f_to(feonumm3n4_t* num, feonumm3n4_t* w, onumm3n4_t* res){

    uint64_t i = 0;
    onumm3n4_t tmp ;

    // Check dimensions.
    feonumm3n4_dimCheck( num, w);

    // res may not come as zero, Modify this?
    onumm3n4_set_r(0,&tmp);

    // The loop for every element in num.
    for (i = 0; i < num->nip; i++){
        
        onumm3n4_gem_oo_to( &w->p_data[i], &num->p_data[i], &tmp, &tmp);

    }

    onumm3n4_set_o(&tmp, res);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm3n4_t feonumm3n4_integrate(feonumm3n4_t* num, feonumm3n4_t* w){

    onumm3n4_t  res = onumm3n4_init(); // res = 0.0

    feonumm3n4_integrate_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm3n4_integrate_to(feonumm3n4_t* num, feonumm3n4_t* w, onumm3n4_t* res){

    uint64_t i = 0;
    onumm3n4_t tmp ;

    // Check dimensions.
    feonumm3n4_dimCheck( num, w);

    // res may not come as zero, Modify this?
    onumm3n4_set_r(0,&tmp);

    // res may not come as zero, Modify this?

    // The loop for every element in num.
    for (i = 0; i < num->nip; i++){
        
        onumm3n4_gem_oo_to( &w->p_data[i], &num->p_data[i], &tmp, &tmp);

    }

    onumm3n4_set_o(&tmp, res);
}
// ----------------------------------------------------------------------------------------------------