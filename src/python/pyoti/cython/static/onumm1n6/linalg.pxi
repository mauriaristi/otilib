...
cpdef dot_product(object lhs, object rhs, object out = None):
  ...
  if   tlhs is feomatm1n6:

    res = __dot_product_FX( lhs, rhs, out = out)

  elif tlhs is omatm1n6:
...
cpdef dot(object lhs, object rhs, object out = None):
  ...
  cdef omatm1n6      Olhs, Orhs, Ores
  cdef oarrm1n6_t   cOres
  cdef dmat       Rlhs, Rrhs, Rres
  cdef darr_t    cRres
  cdef feomatm1n6    Flhs, Frhs, Fres
  ...
  if   tlhs is feomatm1n6:
    ...
    Flhs = lhs
    if   trhs is feomatm1n6: # FF
      Frhs = rhs
      if res_flag:
        Fres = out
        feoarrm1n6_matmul_FF_to( &Flhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm1n6_matmul_FF( &Flhs.arr, &Frhs.arr )
        res = feomatm1n6.create(&cFres)
      # end if 

    elif trhs is omatm1n6:   # FO

      Orhs = rhs
      if res_flag:
        Fres = out
        feoarrm1n6_matmul_FO_to( &Flhs.arr, &Orhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm1n6_matmul_FO( &Flhs.arr, &Orhs.arr )
        res = feomatm1n6.create(&cFres)
      # end if 
      ...
      if res_flag:
        Fres = out
        feoarrm1n6_matmul_FR_to( &Flhs.arr, &Rrhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm1n6_matmul_FR( &Flhs.arr, &Rrhs.arr )
        res = feomatm1n6.create(&cFres)
      # end if 
    ...
    Olhs = lhs
    if   trhs is feomatm1n6: # OF
      Frhs = rhs
      if res_flag:
        Fres = out
        feoarrm1n6_matmul_OF_to( &Olhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm1n6_matmul_OF( &Olhs.arr, &Frhs.arr )
        res = feomatm1n6.create(&cFres)
      # end if 

    elif trhs is omatm1n6:   # OO

      Orhs = rhs
      if res_flag:
        Ores = out
        oarrm1n6_matmul_OO_to( &Olhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = oarrm1n6_matmul_OO( &Olhs.arr, &Orhs.arr )
      ...
      if res_flag:
        Ores = out
        oarrm1n6_matmul_OR_to( &Olhs.arr, &Rrhs.arr ,&Ores.arr)
      else:
        cOres = oarrm1n6_matmul_OR( &Olhs.arr, &Rrhs.arr )
        res = omatm1n6.create(&cOres)
      # end if 
    ...
    Rlhs = lhs
    if   trhs is feomatm1n6: # RF
      Frhs = rhs
      if res_flag:
        Fres = out
        feoarrm1n6_matmul_RF_to( &Rlhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm1n6_matmul_RF( &Rlhs.arr, &Frhs.arr )
        res = feomatm1n6.create(&cFres)
      # end if 

    elif trhs is omatm1n6:   # RO
      Orhs = rhs
      if res_flag:
        Ores = out
        oarrm1n6_matmul_RO_to( &Rlhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = oarrm1n6_matmul_RO( &Rlhs.arr, &Orhs.arr )
        res = omatm1n6.create(&cOres)
      # end if 
...
cpdef transpose(object arr, object out = None):
  ...
  if   tarr is feomatm1n6:
    
    F = arr
    if res_flag:
      Fres = out
      feoarrm1n6_transpose_to( &F.arr, &Fres.arr)
    else:
      cFres = feoarrm1n6_transpose( &F.arr)
      res = feomatm1n6.create(&cFres)
    # end if 

  elif tarr is omatm1n6:

    O = arr
    if res_flag:
      Ores = out
      oarrm1n6_transpose_to( &O.arr, &Ores.arr)
    else:
      cOres = oarrm1n6_transpose( &O.arr)
      res = omatm1n6.create(&cOres)
    # end if    
...
cpdef det(object arr, object out = None):
  ...
  if   tarr is feomatm1n6:
    ...
    if res_flag:
      fres = out
      feoarrm1n6_det_to( &F.arr, &fres.num)
    else:
      cfres = feoarrm1n6_det( &F.arr)
    ...
    if res_flag:      
      ores = out
      oarrm1n6_det_to( &O.arr, &ores.num)
    else:
      cores = oarrm1n6_det( &O.arr)
...
cpdef norm(object arr, coeff_t p = 2.0, object out = None):
  ...
  if   tarr is feomatm1n6:
    ...
    if res_flag:
      fres = out
      feoarrm1n6_pnorm_to( &F.arr, p, &fres.num)
    else:
      cfres = feoarrm1n6_pnorm( &F.arr, p)
      res = feonumm1n6.create(&cfres)
    # end if 
    ...
    if res_flag:
      ores = out
      oarrm1n6_pnorm_to( &O.arr, p, &ores.num)
    else:
      cores = oarrm1n6_pnorm( &O.arr, p)
      res = onumm1n6.create(&cores)
    # end if    
...
cpdef inv(object arr, object out = None):
  ...
  if   tarr is feomatm1n6:    
    ...
    if res_flag:
      Fres = out
      feoarrm1n6_invert_to( &F.arr, &Fres.arr)
    else:
      cFres = feoarrm1n6_invert( &F.arr)
      res = feomatm1n6.create(&cFres)
    # end if 
  elif tarr is omatm1n6:
    O = arr
    if res_flag:
      Ores = out
      oarrm1n6_invert_to( &O.arr, &Ores.arr)
    else:
      cOres = oarrm1n6_invert( &O.arr)
      res = omatm1n6.create(&cOres)
    # end if
  elif tarr is dmat:    
    R = arr
  ...
  if   tarr is omatm1n6:    
    ...
    for i in range(Ores.arr.nrows):
      for j in range(Ores.arr.ncols):
        ...
        oarrm1n6_set_item_ij_r( inverse[i,j], i, j, &Ores.arr)
      # end for
    # end for    
...
#*****************************************************************************************************
cpdef solve(object K_in, omatm1n6 b_in, omatm1n6 out = None, solver = 'SuperLU',solver_args = {}):
  """
...
#*****************************************************************************************************
cdef solve_dense(omatm1n6 K_in, omatm1n6 b_in, omatm1n6 out = None, solver = 'SuperLU',solver_args = {}):
  """
  ...
  for i in range(Ores.nrows):      
    for j in range(Ores.ncols):
      ...
      oarrm1n6_set_item_ij_r( rhs[i,j], i, j, &Ores.arr)
...
#*****************************************************************************************************
cpdef solve_sparse_old(csr_matrix K_in, omatm1n6 b_in, omatm1n6 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cdef solve_sparse(csr_matrix K_in, omatm1n6 b_in, omatm1n6 out = None, solver = 'SuperLU', solver_args = {}):
  """
  ...
  cdef omatm1n6      O, Ores, Otmp, tmp, tmp2, tmp3
...
#*****************************************************************************************************
def solve_sparse_out(csr_matrix K_in, omatm1n6 b_in, omatm1n6 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cpdef get_order_im_array(ord_t ordi, omatm1n6 tmp):
  """
  ...
  cdef np.ndarray res
  cdef onumm1n6_t otmp
  cdef ord_t order
  cdef imdir_t maxidx = 0, idx
  cdef uint64_t i, j, k

  maxidx = onumm1n6_get_ndir_order(ordi,&tmp.arr.p_data[0])
  order  = onumm1n6_get_order( &tmp.arr.p_data[0] )
    ...
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        for k in range( maxidx ): 
          ...
          idx = onumm1n6_get_indx(k, ordi)
          res[ i, j + tmp.ncols * k ] = onumm1n6_get_item( idx, ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array(ord_t ordi, np.ndarray arr, omatm1n6 tmp):
  """
  ...
  for i in range(tmp.nrows):
    ...
    for j in range(tmp.ncols):
      ...
      for k in range( nnz ):          
        ...
        onumm1n6_set_item( val, idx, ordi, &otmp)

      # end for

      tmp[i,j] += onumm1n6.create( &otmp)
...
#*****************************************************************************************************
cpdef get_order_im_array_2(ord_t ordi, omatm1n6 tmp):
  """
  PURPOSE:   Get a specific order from .
  """
  #***************************************************************************************************
  ...
  for i in range(tmp.nrows):
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        coeff_list = onumm1n6_get_order_address( ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array_2(ord_t ordi, np.ndarray arr, omatm1n6 tmp):
  """
  PURPOSE:   Set a specific order from an array.
  """
  #***************************************************************************************************
