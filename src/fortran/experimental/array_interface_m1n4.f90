...
CONTAINS
    ...
    SUBROUTINE pytorch_graph_eval_m1n4(graph, stack_size, c_arr, c_size,x_matso,w_matso) BIND(C)
        ! Test to pass a graph as a rank-2 int64 array from C.
        IMPLICIT NONE

        ! subroutine inputs.
        INTEGER, INTENT(IN) :: stack_size, c_size  !< Array dimensions.
        TYPE(oarrm1n4_t), INTENT(IN) :: x_matso      !< input arrays. (Contains dimensions)
        TYPE(oarrm1n4_t), INTENT(INOUT) :: w_matso   !< output arrays. (Contains dimensions)
        ...
        TYPE(onumm1n4), POINTER :: x_arr(:,:)
        TYPE(onumm1n4), POINTER :: w_arr(:,:)

        TYPE(onumm1n4), POINTER :: p1_ptr(:) ! %%%%%% added new pointer variable.
        TYPE(onumm1n4), POINTER :: p2_ptr(:) ! %%%%%% added new pointer variable.
    ...
    END SUBROUTINE pytorch_graph_eval_m1n4

    SUBROUTINE sub_oo_vv_to(lhs,rhs,res)
      IMPLICIT NONE
      TYPE(onumm1n4), INTENT(IN) :: lhs(:)
      TYPE(onumm1n4), INTENT(IN) :: rhs(SIZE(lhs,1))
      TYPE(onumm1n4), INTENT(INOUT) :: res(SIZE(lhs,1))
   ...
   SUBROUTINE div_oo_vv_to(lhs,rhs,res)
      ...
      DO i=1,SIZE(lhs,1)
          ...
          ! Order 1
          res(i)%e1 = (LHS(i)%E1*RHS(i)%R - LHS(i)%R*RHS(i)%E1)*(1/(RHS(i)%R*RHS(i)%R))

          ! Order 2
          res(i)%e11 = ((-LHS(i)%R*RHS(i)%R*RHS(i)%E11 + LHS(i)%R*RHS(i)%E1**2 - LHS(i)%E1*RHS(i)%R*RHS(i)%E1 + LHS(i)%E11*RHS(i)%R**2))*(1/(RHS(i)%R*RHS(i)%R*RHS(i)%R)) 

          ! Order 3
          res(i)%e111 = - ((LHS(i)%R*RHS(i)%R**2*RHS(i)%E111 - 2*LHS(i)%R*RHS(i)%R*RHS(i)%E1*RHS(i)%E11 + LHS(i)%R*RHS(i)%E1**3 + LHS(i)%E1*RHS(i)%R**2*RHS(i)%E11 - LHS(i)%E1*RHS(i)%R*RHS(i)%E1**2 + LHS(i)%E11*RHS(i)%R**2*RHS(i)%E1 - LHS(i)%E111*RHS(i)%R**3))*(1/(RHS(i)%R*RHS(i)%R*RHS(i)%R*RHS(i)%R))
         ! Order 4
          res(i)%e1111 = ((LHS(i)%R*(2*RHS(i)%R**2*RHS(i)%E1*RHS(i)%E111 + RHS(i)%R**2*(RHS(i)%E11**2 - RHS(i)%R*RHS(i)%E1111) - 3*RHS(i)%R*RHS(i)%E1**2*RHS(i)%E11 + RHS(i)%E1**4) - LHS(i)%E1*RHS(i)%R*(RHS(i)%R**2*RHS(i)%E111 - 2*RHS(i)%R*RHS(i)%E1*RHS(i)%E11 + RHS(i)%E1**3) + LHS(i)%E11*RHS(i)%R**2*(RHS(i)%E1**2 - RHS(i)%R*RHS(i)%E11) - LHS(i)%E111*RHS(i)%R**3*RHS(i)%E1 + LHS(i)%E1111*RHS(i)%R**4))*(1/(RHS(i)%R*RHS(i)%R*RHS(i)%R*RHS(i)%R*RHS(i)%R))
   ...
   SUBROUTINE sin_v_to(X,RES)
      ...
      TYPE(onumm1n4), INTENT(INOUT) :: RES(SIZE(x,1))
...
END MODULE array_interface_m1n4