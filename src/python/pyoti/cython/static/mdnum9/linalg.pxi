...
cpdef dot(object lhs, object rhs, object out = None):
  ...
  cdef mdmat9      Olhs, Orhs, Ores
  cdef mdarr9_t   cOres
  cdef dmat       Rlhs, Rrhs, Rres
  cdef darr_t    cRres
  cdef mdmatfe9    Flhs, Frhs, Fres
  ...
  if   tlhs is mdmatfe9:
    ...
    if   trhs is mdmatfe9: # FF
      ...
      if res_flag:
        Fres = out
        femdarr9_matmul_FF_to( &Flhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr9_matmul_FF( &Flhs.arr, &Frhs.arr )
      ...
      if res_flag:
        Fres = out
        femdarr9_matmul_FO_to( &Flhs.arr, &Orhs.arr ,&Fres.arr)
      else:
        cFres = femdarr9_matmul_FO( &Flhs.arr, &Orhs.arr )
        res = mdmatfe9.create(&cFres)
      # end if 
      ...
      if res_flag:
        Fres = out
        femdarr9_matmul_FR_to( &Flhs.arr, &Rrhs.arr ,&Fres.arr)
      else:
        cFres = femdarr9_matmul_FR( &Flhs.arr, &Rrhs.arr )
        res = mdmatfe9.create(&cFres)
      # end if 
    ...
    if   trhs is mdmatfe9: # OF
      ...
      if res_flag:
        Fres = out
        femdarr9_matmul_OF_to( &Olhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr9_matmul_OF( &Olhs.arr, &Frhs.arr )
        res = mdmatfe9.create(&cFres)
      # end if 
      ...
      if res_flag:
        Ores = out
        mdarr9_matmul_OO_to( &Olhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = mdarr9_matmul_OO( &Olhs.arr, &Orhs.arr )
      ...
      if res_flag:
        Ores = out
        mdarr9_matmul_OR_to( &Olhs.arr, &Rrhs.arr ,&Ores.arr)
      else:
        cOres = mdarr9_matmul_OR( &Olhs.arr, &Rrhs.arr )
        res = mdmat9.create(&cOres)
      # end if 
    ...
    if   trhs is mdmatfe9: # RF
      ...
      if res_flag:
        Fres = out
        femdarr9_matmul_RF_to( &Rlhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr9_matmul_RF( &Rlhs.arr, &Frhs.arr )
        res = mdmatfe9.create(&cFres)
      # end if 
      ...
      if res_flag:
        Ores = out
        mdarr9_matmul_RO_to( &Rlhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = mdarr9_matmul_RO( &Rlhs.arr, &Orhs.arr )
        res = mdmat9.create(&cOres)
      # end if 
...
cpdef transpose(object arr, object out = None):
  ...
  if   tarr is mdmatfe9:
    ...
    if res_flag:
      Fres = out
      femdarr9_transpose_to( &F.arr, &Fres.arr)
    else:
      cFres = femdarr9_transpose( &F.arr)
      res = mdmatfe9.create(&cFres)
    # end if 
    ...
    if res_flag:
      Ores = out
      mdarr9_transpose_to( &O.arr, &Ores.arr)
    else:
      cOres = mdarr9_transpose( &O.arr)
      res = mdmat9.create(&cOres)
    # end if    
...
cpdef det(object arr, object out = None):
  ...
  if   tarr is mdmatfe9:
    ...
    if res_flag:
      fres = out
      femdarr9_det_to( &F.arr, &fres.num)
    else:
      cfres = femdarr9_det( &F.arr)
    ...
    if res_flag:      
      ores = out
      mdarr9_det_to( &O.arr, &ores.num)
    else:
      cores = mdarr9_det( &O.arr)
...
cpdef norm(object arr, coeff_t p = 2.0, object out = None):
  ...
  if   tarr is mdmatfe9:
    ...
    if res_flag:
      fres = out
      femdarr9_pnorm_to( &F.arr, p, &fres.num)
    else:
      cfres = femdarr9_pnorm( &F.arr, p)
      res = mdnumfe9.create(&cfres)
    # end if 
    ...
    if res_flag:
      ores = out
      mdarr9_pnorm_to( &O.arr, p, &ores.num)
    else:
      cores = mdarr9_pnorm( &O.arr, p)
      res = mdnum9.create(&cores)
    # end if    
...
cpdef inv(object arr, object out = None):
  ...
  if   tarr is mdmatfe9:    
    ...
    if res_flag:
      Fres = out
      femdarr9_invert_to( &F.arr, &Fres.arr)
    else:
      cFres = femdarr9_invert( &F.arr)
      res = mdmatfe9.create(&cFres)
    # end if 
  elif tarr is mdmat9:
    O = arr
    if res_flag:
      Ores = out
      mdarr9_invert_to( &O.arr, &Ores.arr)
    else:
      cOres = mdarr9_invert( &O.arr)
      res = mdmat9.create(&cOres)
    # end if
  elif tarr is dmat:    
    R = arr
  ...
  if   tarr is mdmat9:    
    ...
    for i in range(Ores.arr.nrows):
      for j in range(Ores.arr.ncols):
        ...
        mdarr9_set_item_ij_r( inverse[i,j], i, j, &Ores.arr)
      # end for
    # end for    
...
#*****************************************************************************************************
cpdef solve(object K_in, mdmat9 b_in, mdmat9 out = None, solver = 'SuperLU',solver_args = {}):
  """
...
#*****************************************************************************************************
cdef solve_dense(mdmat9 K_in, mdmat9 b_in, mdmat9 out = None, solver = 'SuperLU',solver_args = {}):
  """
  ...
  for i in range(Ores.nrows):      
    for j in range(Ores.ncols):
      ...
      mdarr9_set_item_ij_r( rhs[i,j], i, j, &Ores.arr)
...
#*****************************************************************************************************
cpdef solve_sparse_old(csr_matrix K_in, mdmat9 b_in, mdmat9 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cdef solve_sparse(csr_matrix K_in, mdmat9 b_in, mdmat9 out = None, solver = 'SuperLU', solver_args = {}):
  """
  ...
  cdef mdmat9      O, Ores, Otmp, tmp, tmp2, tmp3
...
#*****************************************************************************************************
def solve_sparse_out(csr_matrix K_in, mdmat9 b_in, mdmat9 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cpdef get_order_im_array(ord_t ordi, mdmat9 tmp):
  """
  ...
  maxidx = mdnum9_get_ndir_order(ordi,&tmp.arr.p_data[0])
  order  = mdnum9_get_order( &tmp.arr.p_data[0] )
    ...
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        for k in range( maxidx ): 
          ...
          idx = mdnum9_get_indx(k, ordi)
          res[ i, j + tmp.ncols * k ] = mdnum9_get_item( idx, ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array(ord_t ordi, np.ndarray arr, mdmat9 tmp):
  """
  ...
  for i in range(tmp.nrows):
    ...
    for j in range(tmp.ncols):
      ...
      for k in range( nnz ):          
        ...
        mdnum9_set_item( val, idx, ordi, &otmp)

      # end for

      tmp[i,j] += mdnum9.create( &otmp)
...
#*****************************************************************************************************
cpdef get_order_im_array_2(ord_t ordi, mdmat9 tmp):
  """
  PURPOSE:   Get a specific order from .
  """
  #***************************************************************************************************
  ...
  for i in range(tmp.nrows):
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        coeff_list = mdnum9_get_order_address( ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array_2(ord_t ordi, np.ndarray arr, mdmat9 tmp):
  """
  PURPOSE:   Set a specific order from an array.
  """
  #***************************************************************************************************
