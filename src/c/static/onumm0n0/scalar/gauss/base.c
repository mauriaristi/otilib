// typedef struct{
//     onumm0n0_t*  p_data;   ///< Data array
//     uint64_t    nip;  ///< Number of integration points.
// } feonumm0n0_t;

// ****************************************************************************************************
ord_t feonumm0n0_get_order( feonumm0n0_t* num ){
    
    ord_t order = 0;
    uint64_t i;

    // Finds the maximum order in the array.
    for( i = 0; i<num->nip; i++ ){
        
        order = MAX(order, onumm0n0_get_order(&num->p_data[i]) );
    
    }

    return order;
    
}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_get_order_im_to( ord_t order, feonumm0n0_t* num, feonumm0n0_t* res){
    
    uint64_t k;

    feonumm0n0_dimCheck(num,res);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_get_order_im_to( order, &num->p_data[k], &res->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_get_order_im( ord_t order, feonumm0n0_t* num){
    
    feonumm0n0_t res;

    res = feonumm0n0_zeros( num->nip);

    feonumm0n0_get_order_im_to( order, num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_extract_im_to( imdir_t idx, ord_t order, feonumm0n0_t* num, feonumm0n0_t* res){
    
    uint64_t k;

    feonumm0n0_dimCheck(num,res);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_extract_im_to( idx, order, &num->p_data[k], &res->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_extract_im( imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    feonumm0n0_t res;

    res = feonumm0n0_zeros( num->nip);

    feonumm0n0_extract_im_to( idx, order, num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_extract_deriv_to( imdir_t idx, ord_t order, feonumm0n0_t* num, feonumm0n0_t* res){
    
    uint64_t k;

    feonumm0n0_dimCheck(num,res);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_extract_deriv_to( idx, order, &num->p_data[k], &res->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_extract_deriv( imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    feonumm0n0_t res;

    res = feonumm0n0_zeros( num->nip);

    feonumm0n0_extract_deriv_to( idx, order, num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_get_deriv_to( imdir_t idx, ord_t order, feonumm0n0_t* num, feonumm0n0_t* res){
    
    uint64_t k;

    feonumm0n0_dimCheck(num,res);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_get_deriv_to( idx, order, &num->p_data[k], &res->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_get_deriv( imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    feonumm0n0_t res;

    res = feonumm0n0_zeros( num->nip);

    feonumm0n0_get_deriv_to( idx, order, num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_get_im_to( imdir_t idx, ord_t order, feonumm0n0_t* num, feonumm0n0_t* res){
    
    uint64_t k;

    feonumm0n0_dimCheck(num,res);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_get_im_to_o( idx, order, &num->p_data[k], &res->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_get_im( imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    feonumm0n0_t res;

    res = feonumm0n0_zeros( num->nip);

    feonumm0n0_get_im_to( idx, order, num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_truncate_im_to( imdir_t idx, ord_t order, feonumm0n0_t* num,  feonumm0n0_t* res){
    
    uint64_t k;

    feonumm0n0_dimCheck(num,res);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_truncate_im_to( idx, order, &num->p_data[k], &res->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_truncate_im( imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    feonumm0n0_t res;

    res = feonumm0n0_zeros( num->nip);

    feonumm0n0_truncate_im_to( idx, order, num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_im_r( coeff_t val, imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    uint64_t k;

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_set_im_r( val, idx, order, &num->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_im_o( onumm0n0_t* val, imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    uint64_t k;

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_set_im_o( val, idx, order, &num->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_im_f( feonumm0n0_t* val, imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    uint64_t k;

    feonumm0n0_dimCheck(val,num);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_set_im_o( &val->p_data[k], idx, order, &num->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_deriv_r( coeff_t val, imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    uint64_t k;

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_set_deriv_r( val, idx, order, &num->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_deriv_o( onumm0n0_t* val, imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    uint64_t k;

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_set_deriv_o( val, idx, order, &num->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_deriv_f( feonumm0n0_t* val, imdir_t idx, ord_t order, feonumm0n0_t* num){
    
    uint64_t k;

    feonumm0n0_dimCheck(val,num);

    for (k = 0; k<num->nip; k++ ){
        
        onumm0n0_set_deriv_o( &val->p_data[k], idx, order, &num->p_data[k]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_to( feonumm0n0_t* src, feonumm0n0_t* dst){
    
    uint64_t i;

    feonumm0n0_dimCheck(src,dst);

    for (i = 0; i<src->nip; i++ ){
        
        onumm0n0_copy_to( &src->p_data[i], &dst->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_copy( feonumm0n0_t* src){
    
    feonumm0n0_t res = feonumm0n0_init();

    res = feonumm0n0_createEmpty(src->nip);

    feonumm0n0_copy_to( src, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_copy_to( feonumm0n0_t* src, feonumm0n0_t* dst){
    
    uint64_t i;

    feonumm0n0_dimCheck(src,dst);

    for (i = 0; i<src->nip; i++ ){
        
        onumm0n0_copy_to( &src->p_data[i], &dst->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_f( feonumm0n0_t* num, feonumm0n0_t* res){
    
    feonumm0n0_copy_to(num,res);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_o( onumm0n0_t* num, feonumm0n0_t* res){
    
    uint64_t i;

    for (i = 0; i<res->nip; i++ ){
        
        onumm0n0_set_o( num, &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_r( coeff_t num, feonumm0n0_t* res){
    
    uint64_t i;

    for (i = 0; i<res->nip; i++ ){
        
        onumm0n0_set_r( num, &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_item_k_o( onumm0n0_t* num, uint64_t k, feonumm0n0_t* res){
    
    if (k < res->nip){

        onumm0n0_set_o(num, &res->p_data[k]); 

    } else {

        printf("ERROR: Index out of bounds in feonumm0n0_get_item_k.\n");
        exit(OTI_BadIndx); // TODO: Raise error instead of quitting the program.

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_set_item_k_r(   coeff_t  num, uint64_t k, feonumm0n0_t* res){

    if (k < res->nip){

        onumm0n0_set_r(num, &res->p_data[k]); 

    } else {

        printf("ERROR: Index out of bounds in feonumm0n0_get_item_k.\n");
        exit(OTI_BadIndx); // TODO: Raise error instead of quitting the program.

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm0n0_t feonumm0n0_get_item_k(feonumm0n0_t* num, uint64_t k){

    onumm0n0_t  res ;
    
    // TODO: Check dimensions
    
    if (k < num->nip){

        res = num->p_data[k];

    } else {

        printf("ERROR: Index out of bounds in feonumm0n0_get_item_k.\n");
        exit(OTI_BadIndx); // TODO: Raise error instead of quitting the program.

    }

    // This is returned uninitialized.
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_get_item_k_to(feonumm0n0_t* num, uint64_t k, onumm0n0_t* res){
    
    
    if (k < num->nip){

        onumm0n0_copy_to( &num->p_data[k], res);

    } else {

        printf("ERROR: Index out of bounds in feonumm0n0_get_item_k.\n");
        exit(OTI_BadIndx); // TODO: Raise error instead of quitting the program.

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm0n0_t feonumm0n0_zeros(uint64_t nip){

    feonumm0n0_t  res  = feonumm0n0_createEmpty(nip);
    
    feonumm0n0_set_r( 0.0, &res);

    // This is returned uninitialized.
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
inline feonumm0n0_t feonumm0n0_createEmpty(uint64_t nip){

    feonumm0n0_t  res ;  
    void * memory = NULL;

    // Allocate memory and check if correctly allocated.
    memory = malloc( nip * sizeof( onumm0n0_t ) );

    if ( memory == NULL ){
        printf("ERROR: Not enough memory to handle array of feonumm0n0.\n Exiting...\n");
        exit(OTI_OutOfMemory); // TODO: Raise error instead of quitting the program.
    }
    
    res.p_data = ( onumm0n0_t* ) memory;
    res.nip = nip;

    // This is returned uninitialized.
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm0n0_free(feonumm0n0_t* num){

    // Free all memory within each coefficient in p_array

    if (num->p_data != NULL){
        // Free pointer 
        free(num->p_data);
    }

    *num = feonumm0n0_init();
    
}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
inline feonumm0n0_t feonumm0n0_init(void){
    
    feonumm0n0_t res;
    
    res.p_data  = NULL;    
    res.nip = 0;

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
inline feonumm0n0_t feonumm0n0_empty_like(feonumm0n0_t* arr){

    feonumm0n0_t res = feonumm0n0_createEmpty(arr->nip);

    return res;

}
// ----------------------------------------------------------------------------------------------------

