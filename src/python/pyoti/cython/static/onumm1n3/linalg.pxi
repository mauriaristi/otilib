...
#*****************************************************************************************************
cpdef solve(object K_in, omatm1n3 b_in, omatm1n3 out = None, solver = 'SuperLU',solver_args = {}):
  """
...
#*****************************************************************************************************
cdef solve_dense(omatm1n3 K_in, omatm1n3 b_in, omatm1n3 out = None, solver = 'SuperLU',solver_args = {}):
  """
...
#*****************************************************************************************************
cpdef solve_sparse_old(csr_matrix K_in, omatm1n3 b_in, omatm1n3 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cdef solve_sparse(csr_matrix K_in, omatm1n3 b_in, omatm1n3 out = None, solver = 'SuperLU', solver_args = {}):
  """
  ...
  cdef omatm1n3      O, Ores, Otmp, tmp, tmp2, tmp3
...
#*****************************************************************************************************
def solve_sparse_out(csr_matrix K_in, omatm1n3 b_in, omatm1n3 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
cpdef get_order_im_array(ord_t ordi, omatm1n3 tmp):
  ...
  maxidx = onumm1n3_get_ndir_order(ordi,&tmp.arr.p_data[0])
  order  = onumm1n3_get_order( &tmp.arr.p_data[0] )
    ...
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        for k in range( maxidx ): 
          ...
          idx = onumm1n3_get_indx(k, ordi)
          res[ i, j + tmp.ncols * k ] = onumm1n3_get_item( idx, ordi, &otmp)
  ...
  for i in range(tmp.nrows):
    ...
    for j in range(tmp.ncols):
      ...
      for k in range( nnz ):          
        ...
        onumm1n3_set_item( val, idx, ordi, &otmp)

      # end for

      tmp[i,j] += onumm1n3.create( &otmp)
...
#*****************************************************************************************************
cpdef get_order_im_array_2(ord_t ordi, omatm1n3 tmp):
  """
  PURPOSE:   Get a specific order from .
  """
  #***************************************************************************************************
  ...
  for i in range(tmp.nrows):
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        coeff_list = onumm1n3_get_order_address( ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array_2(ord_t ordi, np.ndarray arr, omatm1n3 tmp):
  """
  PURPOSE:   Set a specific order from an array.
  """
  #***************************************************************************************************
