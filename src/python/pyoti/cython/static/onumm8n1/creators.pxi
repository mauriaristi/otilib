...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm8n1 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm8n1
    ores = onumm8n1_create_r(0.0)

    # Set the coefficient to 1.
    onumm8n1_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm8n1.create(&ores)

  else:
    
    fres = feonumm8n1(0.0, nip)
    feonumm8n1_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm8n1
    ores = onumm8n1_create_r(1.0)
    res = onumm8n1.create(&ores)

  else:
    
    fres = feonumm8n1(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm8n1
    ores = onumm8n1_create_r(num)
    res = onumm8n1.create(&ores)

  else:
    
    fres = feonumm8n1(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm8n1.
    Ores = oarrm8n1_eye(size)

    return omatm8n1.create(&Ores)

  else:

    # This will create a feomatm8n1
    Fres = feoarrm8n1_eye(size,nip)

    return feomatm8n1.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  cdef oarrm8n1_t   Oarr
  cdef feoarrm8n1_t Farr
  cdef omatm8n1     Ores
  cdef feomatm8n1   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm8n1) or (tin is feomatm8n1):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm8n1_zeros( nrows, ncols)
        Ores = omatm8n1.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm8n1_zeros( nrows, ncols, nip)
        Fres = feomatm8n1.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm8n1.
    Ores = oarrm8n1_zeros(nrows,ncols)
    ...
    # This will create a feomatm8n1
    Fres = feoarrm8n1_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm8n1.
    Ores = oarrm8n1_ones(nrows,ncols)
    ...
    # This will create a feomatm8n1
    Fres = feoarrm8n1_ones(nrows,ncols,nip)
           # feoarrm8n1_ones(nrows,ncols,nip,nbases,order)
