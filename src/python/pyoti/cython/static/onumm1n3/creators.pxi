...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm1n3 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm1n3
    ores = onumm1n3_create_r(0.0)

    # Set the coefficient to 1.
    onumm1n3_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm1n3.create(&ores)

  else:
    
    fres = feonumm1n3(0.0, nip)
    feonumm1n3_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n3
    ores = onumm1n3_create_r(1.0)
    res = onumm1n3.create(&ores)

  else:
    
    fres = feonumm1n3(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n3
    ores = onumm1n3_create_r(num)
    res = onumm1n3.create(&ores)

  else:
    
    fres = feonumm1n3(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n3.
    Ores = oarrm1n3_eye(size)

    return omatm1n3.create(&Ores)

  else:

    # This will create a feomatm1n3
    Fres = feoarrm1n3_eye(size,nip)

    return feomatm1n3.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  cdef oarrm1n3_t   Oarr
  cdef feoarrm1n3_t Farr
  cdef omatm1n3     Ores
  cdef feomatm1n3   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm1n3) or (tin is feomatm1n3):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm1n3_zeros( nrows, ncols)
        Ores = omatm1n3.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm1n3_zeros( nrows, ncols, nip)
        Fres = feomatm1n3.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n3.
    Ores = oarrm1n3_zeros(nrows,ncols)
    ...
    # This will create a feomatm1n3
    Fres = feoarrm1n3_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n3.
    Ores = oarrm1n3_ones(nrows,ncols)
    ...
    # This will create a feomatm1n3
    Fres = feoarrm1n3_ones(nrows,ncols,nip)
           # feoarrm1n3_ones(nrows,ncols,nip,nbases,order)
