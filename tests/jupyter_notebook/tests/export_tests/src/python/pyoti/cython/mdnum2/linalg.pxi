...
cpdef dot_product(object lhs, object rhs, object out = None):
  ...
  if   tlhs is femdarr2_t:

    res = __dot_product_FX( lhs, rhs, out = out)

  elif tlhs is mdarr2_t:
...
cpdef dot(object lhs, object rhs, object out = None):
  ...
  cdef mdarr2_t      Olhs, Orhs, Ores
  cdef mdarr2_t   cOres
  cdef dmat       Rlhs, Rrhs, Rres
  cdef darr_t    cRres
  cdef femdarr2_t    Flhs, Frhs, Fres
  cdef femdarr2_t cFres

  cdef csr_mdarr2_t  Slhs
  ...
  elif tlhs is csr_mdarr2_t:

    # Slhs = lhs
    if   trhs is mdarr2_t: # SO
      if res_flag:
        Ores = out
        csrmdarr2_t_matmul_SO_to( lhs, rhs, Ores)
      else:
        res = csrmdarr2_t_matmul_SO( lhs, rhs)
      # end if 
...
#*****************************************************************************************************
cpdef solve(object K_in, mdarr2_t b_in, mdarr2_t out = None):
  """
  ...
  cdef mdarr2_t      O, Ores, Otmp
  cdef csr_mdarr2_t  S, Sres, Stmp  
  cdef uint64_t i,j,k,l
  cdef ord_t ordi, ord_lhs, ord_rhs, Oord
  cdef uint8_t res_flag = 1
  cdef object res
...
#*****************************************************************************************************
cdef solve_dense(mdarr2_t K_in, mdarr2_t b_in, mdarr2_t out = None):
  """
...
#*****************************************************************************************************
cdef solve_sparse(csr_mdarr2_t K_in, mdarr2_t b_in, mdarr2_t out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cpdef get_order_im_array(ord_t ordi, mdarr2_t tmp):
  """
...
#*****************************************************************************************************
cpdef set_order_im_from_array(ord_t ordi, np.ndarray arr, mdarr2_t tmp):
  """
  ...
  for i in range(tmp.nrows):
    ...
    for j in range(tmp.ncols):
      ...
      tmp[i,j] += mdnum2_t.create( &otmp, FLAGS = 0)
