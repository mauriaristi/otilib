...
cpdef e( object hum_dir, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm2n1 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm2n1
    ores = onumm2n1_create_r(0.0)

    # Set the coefficient to 1.
    onumm2n1_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm2n1.create(&ores)

  else:
    
    fres = feonumm2n1(0.0, nip)
    feonumm2n1_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm2n1
    ores = onumm2n1_create_r(1.0)
    res = onumm2n1.create(&ores)

  else:
    
    fres = feonumm2n1(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm2n1
    ores = onumm2n1_create_r(num)
    res = onumm2n1.create(&ores)

  else:
    
    fres = feonumm2n1(num, nip)
    res = fres
...
cpdef eye(uint64_t size, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm2n1.
    Ores = oarrm2n1_eye(size)

    return omatm2n1.create(&Ores)

  else:

    # This will create a feomatm2n1
    Fres = feoarrm2n1_eye(size,nip)

    return feomatm2n1.create(&Fres)
...
cpdef array( object arr,  uint64_t nip = 0):
  ...
  cdef oarrm2n1_t   Oarr
  cdef feoarrm2n1_t Farr
  cdef omatm2n1     Ores
  cdef feomatm2n1   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm2n1) or (tin is feomatm2n1):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm2n1_zeros( nrows, ncols)
        Ores = omatm2n1.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm2n1_zeros( nrows, ncols, nip)
        Fres = feomatm2n1.create(&Farr)
...
cpdef zeros( object shape_in, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm2n1.
    Ores = oarrm2n1_zeros(nrows,ncols)
    ...
    # This will create a feomatm2n1
    Fres = feoarrm2n1_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm2n1.
    Ores = oarrm2n1_ones(nrows,ncols)
    ...
    # This will create a feomatm2n1
    Fres = feoarrm2n1_ones(nrows,ncols,nip)
           # feoarrm2n1_ones(nrows,ncols,nip,nbases,order)
