...
cpdef dot(object lhs, object rhs, object out = None):
  ...
  cdef mdmat6      Olhs, Orhs, Ores
  cdef mdarr6_t   cOres
  cdef dmat       Rlhs, Rrhs, Rres
  cdef darr_t    cRres
  cdef mdmatfe6    Flhs, Frhs, Fres
  ...
  if   tlhs is mdmatfe6:
    ...
    if   trhs is mdmatfe6: # FF
      ...
      if res_flag:
        Fres = out
        femdarr6_matmul_FF_to( &Flhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr6_matmul_FF( &Flhs.arr, &Frhs.arr )
      ...
      if res_flag:
        Fres = out
        femdarr6_matmul_FO_to( &Flhs.arr, &Orhs.arr ,&Fres.arr)
      else:
        cFres = femdarr6_matmul_FO( &Flhs.arr, &Orhs.arr )
        res = mdmatfe6.create(&cFres)
      # end if 
      ...
      if res_flag:
        Fres = out
        femdarr6_matmul_FR_to( &Flhs.arr, &Rrhs.arr ,&Fres.arr)
      else:
        cFres = femdarr6_matmul_FR( &Flhs.arr, &Rrhs.arr )
        res = mdmatfe6.create(&cFres)
      # end if 
    ...
    if   trhs is mdmatfe6: # OF
      ...
      if res_flag:
        Fres = out
        femdarr6_matmul_OF_to( &Olhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr6_matmul_OF( &Olhs.arr, &Frhs.arr )
        res = mdmatfe6.create(&cFres)
      # end if 
      ...
      if res_flag:
        Ores = out
        mdarr6_matmul_OO_to( &Olhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = mdarr6_matmul_OO( &Olhs.arr, &Orhs.arr )
      ...
      if res_flag:
        Ores = out
        mdarr6_matmul_OR_to( &Olhs.arr, &Rrhs.arr ,&Ores.arr)
      else:
        cOres = mdarr6_matmul_OR( &Olhs.arr, &Rrhs.arr )
        res = mdmat6.create(&cOres)
      # end if 
    ...
    if   trhs is mdmatfe6: # RF
      ...
      if res_flag:
        Fres = out
        femdarr6_matmul_RF_to( &Rlhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr6_matmul_RF( &Rlhs.arr, &Frhs.arr )
        res = mdmatfe6.create(&cFres)
      # end if 
      ...
      if res_flag:
        Ores = out
        mdarr6_matmul_RO_to( &Rlhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = mdarr6_matmul_RO( &Rlhs.arr, &Orhs.arr )
        res = mdmat6.create(&cOres)
      # end if 
...
cpdef transpose(object arr, object out = None):
  ...
  if   tarr is mdmatfe6:
    ...
    if res_flag:
      Fres = out
      femdarr6_transpose_to( &F.arr, &Fres.arr)
    else:
      cFres = femdarr6_transpose( &F.arr)
      res = mdmatfe6.create(&cFres)
    # end if 
    ...
    if res_flag:
      Ores = out
      mdarr6_transpose_to( &O.arr, &Ores.arr)
    else:
      cOres = mdarr6_transpose( &O.arr)
      res = mdmat6.create(&cOres)
    # end if    
...
cpdef det(object arr, object out = None):
  ...
  if   tarr is mdmatfe6:
    ...
    if res_flag:
      fres = out
      femdarr6_det_to( &F.arr, &fres.num)
    else:
      cfres = femdarr6_det( &F.arr)
    ...
    if res_flag:      
      ores = out
      mdarr6_det_to( &O.arr, &ores.num)
    else:
      cores = mdarr6_det( &O.arr)
...
cpdef norm(object arr, coeff_t p = 2.0, object out = None):
  ...
  if   tarr is mdmatfe6:
    ...
    if res_flag:
      fres = out
      femdarr6_pnorm_to( &F.arr, p, &fres.num)
    else:
      cfres = femdarr6_pnorm( &F.arr, p)
      res = mdnumfe6.create(&cfres)
    # end if 
    ...
    if res_flag:
      ores = out
      mdarr6_pnorm_to( &O.arr, p, &ores.num)
    else:
      cores = mdarr6_pnorm( &O.arr, p)
      res = mdnum6.create(&cores)
    # end if    
...
cpdef inv(object arr, object out = None):
  ...
  if   tarr is mdmatfe6:    
    ...
    if res_flag:
      Fres = out
      femdarr6_invert_to( &F.arr, &Fres.arr)
    else:
      cFres = femdarr6_invert( &F.arr)
      res = mdmatfe6.create(&cFres)
    # end if 
  elif tarr is mdmat6:
    O = arr
    if res_flag:
      Ores = out
      mdarr6_invert_to( &O.arr, &Ores.arr)
    else:
      cOres = mdarr6_invert( &O.arr)
      res = mdmat6.create(&cOres)
    # end if
  elif tarr is dmat:    
    R = arr
  ...
  if   tarr is mdmat6:    
    ...
    for i in range(Ores.arr.nrows):
      for j in range(Ores.arr.ncols):
        ...
        mdarr6_set_item_ij_r( inverse[i,j], i, j, &Ores.arr)
      # end for
    # end for    
...
#*****************************************************************************************************
cpdef solve(object K_in, mdmat6 b_in, mdmat6 out = None, solver = 'SuperLU',solver_args = {}):
  """
...
#*****************************************************************************************************
cdef solve_dense(mdmat6 K_in, mdmat6 b_in, mdmat6 out = None, solver = 'SuperLU',solver_args = {}):
  """
  ...
  for i in range(Ores.nrows):      
    for j in range(Ores.ncols):
      ...
      mdarr6_set_item_ij_r( rhs[i,j], i, j, &Ores.arr)
...
#*****************************************************************************************************
cpdef solve_sparse_old(csr_matrix K_in, mdmat6 b_in, mdmat6 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cdef solve_sparse(csr_matrix K_in, mdmat6 b_in, mdmat6 out = None, solver = 'SuperLU', solver_args = {}):
  """
  ...
  cdef mdmat6      O, Ores, Otmp, tmp, tmp2, tmp3
...
#*****************************************************************************************************
def solve_sparse_out(csr_matrix K_in, mdmat6 b_in, mdmat6 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cpdef get_order_im_array(ord_t ordi, mdmat6 tmp):
  """
  ...
  maxidx = mdnum6_get_ndir_order(ordi,&tmp.arr.p_data[0])
  order  = mdnum6_get_order( &tmp.arr.p_data[0] )
    ...
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        for k in range( maxidx ): 
          ...
          idx = mdnum6_get_indx(k, ordi)
          res[ i, j + tmp.ncols * k ] = mdnum6_get_item( idx, ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array(ord_t ordi, np.ndarray arr, mdmat6 tmp):
  """
  ...
  for i in range(tmp.nrows):
    ...
    for j in range(tmp.ncols):
      ...
      for k in range( nnz ):          
        ...
        mdnum6_set_item( val, idx, ordi, &otmp)

      # end for

      tmp[i,j] += mdnum6.create( &otmp)
...
#*****************************************************************************************************
cpdef get_order_im_array_2(ord_t ordi, mdmat6 tmp):
  """
  PURPOSE:   Get a specific order from .
  """
  #***************************************************************************************************
  ...
  for i in range(tmp.nrows):
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        coeff_list = mdnum6_get_order_address( ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array_2(ord_t ordi, np.ndarray arr, mdmat6 tmp):
  """
  PURPOSE:   Set a specific order from an array.
  """
  #***************************************************************************************************
