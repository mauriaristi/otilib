...
cpdef dot_product(object lhs, object rhs, object out = None):
  ...
  if   tlhs is feomatm7n2:

    res = __dot_product_FX( lhs, rhs, out = out)

  elif tlhs is omatm7n2:
...
cpdef dot(object lhs, object rhs, object out = None):
  ...
  cdef omatm7n2      Olhs, Orhs, Ores
  cdef oarrm7n2_t   cOres
  cdef dmat       Rlhs, Rrhs, Rres
  cdef darr_t    cRres
  cdef feomatm7n2    Flhs, Frhs, Fres
  ...
  if   tlhs is feomatm7n2:
    ...
    Flhs = lhs
    if   trhs is feomatm7n2: # FF
      Frhs = rhs
      if res_flag:
        Fres = out
        feoarrm7n2_matmul_FF_to( &Flhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm7n2_matmul_FF( &Flhs.arr, &Frhs.arr )
        res = feomatm7n2.create(&cFres)
      # end if 

    elif trhs is omatm7n2:   # FO

      Orhs = rhs
      if res_flag:
        Fres = out
        feoarrm7n2_matmul_FO_to( &Flhs.arr, &Orhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm7n2_matmul_FO( &Flhs.arr, &Orhs.arr )
        res = feomatm7n2.create(&cFres)
      # end if 
      ...
      if res_flag:
        Fres = out
        feoarrm7n2_matmul_FR_to( &Flhs.arr, &Rrhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm7n2_matmul_FR( &Flhs.arr, &Rrhs.arr )
        res = feomatm7n2.create(&cFres)
      # end if 
    ...
    Olhs = lhs
    if   trhs is feomatm7n2: # OF
      Frhs = rhs
      if res_flag:
        Fres = out
        feoarrm7n2_matmul_OF_to( &Olhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm7n2_matmul_OF( &Olhs.arr, &Frhs.arr )
        res = feomatm7n2.create(&cFres)
      # end if 

    elif trhs is omatm7n2:   # OO

      Orhs = rhs
      if res_flag:
        Ores = out
        oarrm7n2_matmul_OO_to( &Olhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = oarrm7n2_matmul_OO( &Olhs.arr, &Orhs.arr )
      ...
      if res_flag:
        Ores = out
        oarrm7n2_matmul_OR_to( &Olhs.arr, &Rrhs.arr ,&Ores.arr)
      else:
        cOres = oarrm7n2_matmul_OR( &Olhs.arr, &Rrhs.arr )
        res = omatm7n2.create(&cOres)
      # end if 
    ...
    Rlhs = lhs
    if   trhs is feomatm7n2: # RF
      Frhs = rhs
      if res_flag:
        Fres = out
        feoarrm7n2_matmul_RF_to( &Rlhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = feoarrm7n2_matmul_RF( &Rlhs.arr, &Frhs.arr )
        res = feomatm7n2.create(&cFres)
      # end if 

    elif trhs is omatm7n2:   # RO
      Orhs = rhs
      if res_flag:
        Ores = out
        oarrm7n2_matmul_RO_to( &Rlhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = oarrm7n2_matmul_RO( &Rlhs.arr, &Orhs.arr )
        res = omatm7n2.create(&cOres)
      # end if 
...
cpdef transpose(object arr, object out = None):
  ...
  if   tarr is feomatm7n2:
    
    F = arr
    if res_flag:
      Fres = out
      feoarrm7n2_transpose_to( &F.arr, &Fres.arr)
    else:
      cFres = feoarrm7n2_transpose( &F.arr)
      res = feomatm7n2.create(&cFres)
    # end if 

  elif tarr is omatm7n2:

    O = arr
    if res_flag:
      Ores = out
      oarrm7n2_transpose_to( &O.arr, &Ores.arr)
    else:
      cOres = oarrm7n2_transpose( &O.arr)
      res = omatm7n2.create(&cOres)
    # end if    
...
cpdef det(object arr, object out = None):
  ...
  if   tarr is feomatm7n2:
    ...
    if res_flag:
      fres = out
      feoarrm7n2_det_to( &F.arr, &fres.num)
    else:
      cfres = feoarrm7n2_det( &F.arr)
    ...
    if res_flag:      
      ores = out
      oarrm7n2_det_to( &O.arr, &ores.num)
    else:
      cores = oarrm7n2_det( &O.arr)
...
cpdef norm(object arr, coeff_t p = 2.0, object out = None):
  ...
  if   tarr is feomatm7n2:
    ...
    if res_flag:
      fres = out
      feoarrm7n2_pnorm_to( &F.arr, p, &fres.num)
    else:
      cfres = feoarrm7n2_pnorm( &F.arr, p)
      res = feonumm7n2.create(&cfres)
    # end if 
    ...
    if res_flag:
      ores = out
      oarrm7n2_pnorm_to( &O.arr, p, &ores.num)
    else:
      cores = oarrm7n2_pnorm( &O.arr, p)
      res = onumm7n2.create(&cores)
    # end if    
...
cpdef inv(object arr, object out = None):
  ...
  if   tarr is feomatm7n2:    
    ...
    if res_flag:
      Fres = out
      feoarrm7n2_invert_to( &F.arr, &Fres.arr)
    else:
      cFres = feoarrm7n2_invert( &F.arr)
      res = feomatm7n2.create(&cFres)
    # end if 
  elif tarr is omatm7n2:
    O = arr
    if res_flag:
      Ores = out
      oarrm7n2_invert_to( &O.arr, &Ores.arr)
    else:
      cOres = oarrm7n2_invert( &O.arr)
      res = omatm7n2.create(&cOres)
    # end if
  elif tarr is dmat:    
    R = arr
  ...
  if   tarr is omatm7n2:    
    ...
    for i in range(Ores.arr.nrows):
      for j in range(Ores.arr.ncols):
        ...
        oarrm7n2_set_item_ij_r( inverse[i,j], i, j, &Ores.arr)
      # end for
    # end for    
...
#*****************************************************************************************************
cpdef solve(object K_in, omatm7n2 b_in, omatm7n2 out = None, solver = 'SuperLU',solver_args = {}):
  """
...
#*****************************************************************************************************
cdef solve_dense(omatm7n2 K_in, omatm7n2 b_in, omatm7n2 out = None, solver = 'SuperLU',solver_args = {}):
  """
  ...
  for i in range(Ores.nrows):      
    for j in range(Ores.ncols):
      ...
      oarrm7n2_set_item_ij_r( rhs[i,j], i, j, &Ores.arr)
...
#*****************************************************************************************************
cpdef solve_sparse_old(csr_matrix K_in, omatm7n2 b_in, omatm7n2 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cdef solve_sparse(csr_matrix K_in, omatm7n2 b_in, omatm7n2 out = None, solver = 'SuperLU', solver_args = {}):
  """
  ...
  cdef omatm7n2      O, Ores, Otmp, tmp, tmp2, tmp3
...
#*****************************************************************************************************
def solve_sparse_out(csr_matrix K_in, omatm7n2 b_in, omatm7n2 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cpdef get_order_im_array(ord_t ordi, omatm7n2 tmp):
  """
  ...
  cdef np.ndarray res
  cdef onumm7n2_t otmp
  cdef ord_t order
  cdef imdir_t maxidx = 0, idx
  cdef uint64_t i, j, k

  maxidx = onumm7n2_get_ndir_order(ordi,&tmp.arr.p_data[0])
  order  = onumm7n2_get_order( &tmp.arr.p_data[0] )
    ...
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        for k in range( maxidx ): 
          ...
          idx = onumm7n2_get_indx(k, ordi)
          res[ i, j + tmp.ncols * k ] = onumm7n2_get_item( idx, ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array(ord_t ordi, np.ndarray arr, omatm7n2 tmp):
  """
  ...
  for i in range(tmp.nrows):
    ...
    for j in range(tmp.ncols):
      ...
      for k in range( nnz ):          
        ...
        onumm7n2_set_item( val, idx, ordi, &otmp)

      # end for

      tmp[i,j] += onumm7n2.create( &otmp)
...
#*****************************************************************************************************
cpdef get_order_im_array_2(ord_t ordi, omatm7n2 tmp):
  """
  PURPOSE:   Get a specific order from .
  """
  #***************************************************************************************************
  ...
  for i in range(tmp.nrows):
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        coeff_list = onumm7n2_get_order_address( ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array_2(ord_t ordi, np.ndarray arr, omatm7n2 tmp):
  """
  PURPOSE:   Set a specific order from an array.
  """
  #***************************************************************************************************
