...
cdef class csr_matrix:
    ...
    if targ1 == lil_matrix :
      ...
      for i in range(self.nrows):
        ...
        for j in range(len(lil.rows[i])):
          ...
          mdnum6_copy_to( &onum.num, &self.arr.p_data[k])
      ...
      if len(arg1) == 3: # Same as in scipy csr sparse.
        ...
        for i in range(nnz):
          ...
          if type(data[i]) == mdnum6:

            onum = data[i]
            mdnum6_copy_to( &onum.num, &self.arr.p_data[i])

          else:
            
            self.arr.p_data[i] = mdnum6_create_r( data[i] )
  ...
  def data(self): 
    ...
    return mdmat6.create(&self.arr, FLAGS=0)
  ...
  def order(self): 
    ...
    order = mdarr6_get_order(&self.arr)
  ...
  def copy(self):
    ...
    res.arr       = mdarr6_zeros( self.nnz, 1)
    ...
    mdarr6_copy_to( &self.arr, &res.arr)
  ...
  cpdef get_deriv(self, object hum_dir ):
    ...
    mdarr6_get_deriv_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef get_im(self, object hum_dir ):
    ...
    mdarr6_get_im_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef extract_im(self, object hum_dir):
    ...
    mdarr6_extract_im_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef  extract_deriv(self, object hum_dir):
    ...
    mdarr6_extract_deriv_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef  get_order_im(self, ord_t order):
    ...
    mdarr6_get_order_im_to( order, &self.arr, &res.arr)
  ...
  cpdef  truncate( self, object hum_dir):
    ...
    mdarr6_truncate_im_to( indx, order, &self.arr, &res.arr)
  ...
  def get_active_bases(self):
    ...
    tmp = mdmat6.create(&self.arr,FLAGS=0)
...
# ****************************************************************************************************
cdef void csrmatrix_matmul_SO_to(csr_matrix lhs, mdmat6 rhs, mdmat6 res):
  """
  ...
  for i in range(lhs.nrows):
    ...
    for j in range(rhs.ncols):
      ...
      for l in range( lhs.p_indptr[i], lhs.p_indptr[i+1] ):
        ...
        mdnum6_gem_oo_to( &lhs.arr.p_data[l],
                        &rhs.arr.p_data[ j + k * rhs.ncols ],
                        &tmp, &tmp)

      # end for

      mdarr6_set_item_ij_o( &tmp, i, j, &res.arr)
...
# ****************************************************************************************************
cdef mdmat6 csrmatrix_matmul_SO(csr_matrix lhs, mdmat6 rhs):
  """
  PORPUSE:  Perform matrix-matrix multiplication between csr matrix and dense matrix.
...
# ****************************************************************************************************
cdef void csrmatrix_trunc_matmul_SO_to(ord_t ord_lhs, csr_matrix lhs, ord_t ord_rhs, mdmat6 rhs, mdmat6 res):
  """
  PORPUSE:  Perform matrix-matrix multiplication between csr matrix and dense matrix.
  ...
  for i in range(lhs.nrows):
    ...
    for j in range(rhs.ncols):
      ...
      for l in range( lhs.p_indptr[i], lhs.p_indptr[i+1] ):
        ...
        mdnum6_trunc_gem_oo_to( ord_lhs, &lhs.arr.p_data[l], 
...
# ****************************************************************************************************
cdef mdmat6 csrmatrix_trunc_matmul_SO(ord_t ord_lhs, csr_matrix lhs, ord_t ord_rhs, mdmat6 rhs):
  """
  PORPUSE:  Perform matrix-matrix multiplication between csr matrix and dense matrix.
