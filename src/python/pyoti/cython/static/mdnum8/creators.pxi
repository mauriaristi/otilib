...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a mdnum8 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a mdnum8
    ores = mdnum8_create_r(0.0)

    # Set the coefficient to 1.
    mdnum8_set_item(1.0, indx_hd, order_hd, &ores) 
    res = mdnum8.create(&ores)

  else:
    
    fres = mdnumfe8(0.0, nip)
    femdnum8_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum8
    ores = mdnum8_create_r(1.0)
    res = mdnum8.create(&ores)

  else:
    
    fres = mdnumfe8(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a mdnum8
    ores = mdnum8_create_r(num)
    res = mdnum8.create(&ores)

  else:
    
    fres = mdnumfe8(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an mdmat8.
    Ores = mdarr8_eye(size)

    return mdmat8.create(&Ores)

  else:

    # This will create a mdmatfe8
    Fres = femdarr8_eye(size,nip)

    return mdmatfe8.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if (tin is mdmat8) or (tin is mdmatfe8):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = mdarr8_zeros( nrows, ncols)
        Ores = mdmat8.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = femdarr8_zeros( nrows, ncols, nip)
        Fres = mdmatfe8.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create a mdmatfe8
    Fres = femdarr8_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create a mdmatfe8
    Fres = femdarr8_ones(nrows,ncols,nip)
           # femdarr8_ones(nrows,ncols,nip,nbases,order)
