...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm5n5 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm5n5
    ores = onumm5n5_create_r(0.0)

    # Set the coefficient to 1.
    onumm5n5_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm5n5.create(&ores)

  else:
    
    fres = feonumm5n5(0.0, nip)
    feonumm5n5_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm5n5
    ores = onumm5n5_create_r(1.0)
    res = onumm5n5.create(&ores)

  else:
    
    fres = feonumm5n5(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm5n5
    ores = onumm5n5_create_r(num)
    res = onumm5n5.create(&ores)

  else:
    
    fres = feonumm5n5(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm5n5.
    Ores = oarrm5n5_eye(size)

    return omatm5n5.create(&Ores)

  else:

    # This will create a feomatm5n5
    Fres = feoarrm5n5_eye(size,nip)

    return feomatm5n5.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  cdef oarrm5n5_t   Oarr
  cdef feoarrm5n5_t Farr
  cdef omatm5n5     Ores
  cdef feomatm5n5   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm5n5) or (tin is feomatm5n5):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm5n5_zeros( nrows, ncols)
        Ores = omatm5n5.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm5n5_zeros( nrows, ncols, nip)
        Fres = feomatm5n5.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm5n5.
    Ores = oarrm5n5_zeros(nrows,ncols)
    ...
    # This will create a feomatm5n5
    Fres = feoarrm5n5_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm5n5.
    Ores = oarrm5n5_ones(nrows,ncols)
    ...
    # This will create a feomatm5n5
    Fres = feoarrm5n5_ones(nrows,ncols,nip)
           # feoarrm5n5_ones(nrows,ncols,nip,nbases,order)
