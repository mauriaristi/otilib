

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_atanh(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);

    feonumm2n7_atanh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_asinh(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);

    feonumm2n7_asinh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_acosh(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);

    feonumm2n7_acosh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_tanh(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);

    feonumm2n7_tanh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_cosh(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_cosh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_sinh(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_sinh_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_asin(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_asin_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_acos(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_acos_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_atan(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_atan_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_tan(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_tan_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_cos(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_cos_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_sin(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_sin_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_logb(feonumm2n7_t* num, double base){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_logb_to(num, base, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_log10(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_log10_to(num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_log(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_log_to(num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_exp(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_exp_to(num, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_cbrt(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);

    feonumm2n7_cbrt_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_sqrt(feonumm2n7_t* num){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);
    
    feonumm2n7_sqrt_to(num, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
feonumm2n7_t feonumm2n7_pow(feonumm2n7_t* num, double e){

    feonumm2n7_t res = feonumm2n7_init();

    res = feonumm2n7_createEmpty(num->nip);

    feonumm2n7_pow_to(num, e, &res);
    
    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_atanh_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_atanh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_asinh_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_asinh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_acosh_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_acosh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_tanh_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_tanh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_cosh_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_cosh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_sinh_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_sinh_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_asin_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_asin_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_acos_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_acos_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_atan_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_atan_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_tan_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_tan_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_cos_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_cos_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_sin_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_sin_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_logb_to(feonumm2n7_t* num, double base, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_logb_to(&num->p_data[i], base, &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_log10_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_log10_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_log_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_log_to(&num->p_data[i], &res->p_data[i]);

    } 

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_exp_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_exp_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_cbrt_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_cbrt_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_sqrt_to(feonumm2n7_t* num, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_sqrt_to(&num->p_data[i], &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_pow_to(feonumm2n7_t* num, double e, feonumm2n7_t* res){

    uint64_t i;

    feonumm2n7_dimCheck(num,res);

    for( i = 0; i<num->nip; i++){
        
        onumm2n7_pow_to(&num->p_data[i], e, &res->p_data[i]);

    }

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm2n7_t feonumm2n7_integrate_r(coeff_t num, feonumm2n7_t* w){

    onumm2n7_t  res = onumm2n7_init(); // res = 0.0

    feonumm2n7_integrate_r_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_integrate_r_to(coeff_t num, feonumm2n7_t* w, onumm2n7_t* res){

    uint64_t i = 0;

    onumm2n7_t tmp ;

    // res may not come as zero, Modify this?
    onumm2n7_set_r(0,&tmp);

    // The loop for every element in num.
    for (i = 0; i < w->nip; i++){
        
        onumm2n7_sum_oo_to(  &tmp, &w->p_data[i], &tmp);

    }

    onumm2n7_mul_ro_to( num, &tmp, res);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm2n7_t feonumm2n7_integrate_o(onumm2n7_t* num, feonumm2n7_t* w){

    onumm2n7_t  res = onumm2n7_init(); // res = 0.0

    feonumm2n7_integrate_o_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_integrate_o_to(onumm2n7_t* num, feonumm2n7_t* w, onumm2n7_t* res){

    uint64_t i = 0;

    onumm2n7_t tmp ;

    // res may not come as zero, Modify this?
    onumm2n7_set_r(0, &tmp);

    // The loop for every element in num.
    for (i = 0; i < w->nip; i++){
        
        onumm2n7_sum_oo_to(  &tmp, &w->p_data[i], &tmp);

    }

    onumm2n7_mul_oo_to( num, &tmp, res);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm2n7_t feonumm2n7_integrate_f(feonumm2n7_t* num, feonumm2n7_t* w){

    onumm2n7_t  res = onumm2n7_init(); // res = 0.0

    feonumm2n7_integrate_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_integrate_f_to(feonumm2n7_t* num, feonumm2n7_t* w, onumm2n7_t* res){

    uint64_t i = 0;
    onumm2n7_t tmp ;

    // Check dimensions.
    feonumm2n7_dimCheck( num, w);

    // res may not come as zero, Modify this?
    onumm2n7_set_r(0,&tmp);

    // The loop for every element in num.
    for (i = 0; i < num->nip; i++){
        
        onumm2n7_gem_oo_to( &w->p_data[i], &num->p_data[i], &tmp, &tmp);

    }

    onumm2n7_set_o(&tmp, res);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
onumm2n7_t feonumm2n7_integrate(feonumm2n7_t* num, feonumm2n7_t* w){

    onumm2n7_t  res = onumm2n7_init(); // res = 0.0

    feonumm2n7_integrate_to(num, w, &res);

    return res;

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void feonumm2n7_integrate_to(feonumm2n7_t* num, feonumm2n7_t* w, onumm2n7_t* res){

    uint64_t i = 0;
    onumm2n7_t tmp ;

    // Check dimensions.
    feonumm2n7_dimCheck( num, w);

    // res may not come as zero, Modify this?
    onumm2n7_set_r(0,&tmp);

    // res may not come as zero, Modify this?

    // The loop for every element in num.
    for (i = 0; i < num->nip; i++){
        
        onumm2n7_gem_oo_to( &w->p_data[i], &num->p_data[i], &tmp, &tmp);

    }

    onumm2n7_set_o(&tmp, res);
}
// ----------------------------------------------------------------------------------------------------