...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm1n4 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm1n4
    ores = onumm1n4_create_r(0.0)

    # Set the coefficient to 1.
    onumm1n4_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm1n4.create(&ores)

  else:
    
    fres = feonumm1n4(0.0, nip)
    feonumm1n4_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n4
    ores = onumm1n4_create_r(1.0)
    res = onumm1n4.create(&ores)

  else:
    
    fres = feonumm1n4(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n4
    ores = onumm1n4_create_r(num)
    res = onumm1n4.create(&ores)

  else:
    
    fres = feonumm1n4(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n4.
    Ores = oarrm1n4_eye(size)

    return omatm1n4.create(&Ores)

  else:

    # This will create a feomatm1n4
    Fres = feoarrm1n4_eye(size,nip)

    return feomatm1n4.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  cdef oarrm1n4_t   Oarr
  cdef feoarrm1n4_t Farr
  cdef omatm1n4     Ores
  cdef feomatm1n4   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm1n4) or (tin is feomatm1n4):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm1n4_zeros( nrows, ncols)
        Ores = omatm1n4.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm1n4_zeros( nrows, ncols, nip)
        Fres = feomatm1n4.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n4.
    Ores = oarrm1n4_zeros(nrows,ncols)
    ...
    # This will create a feomatm1n4
    Fres = feoarrm1n4_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n4.
    Ores = oarrm1n4_ones(nrows,ncols)
    ...
    # This will create a feomatm1n4
    Fres = feoarrm1n4_ones(nrows,ncols,nip)
           # feoarrm1n4_ones(nrows,ncols,nip,nbases,order)
