...
cpdef e( object hum_dir, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm9n1 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm9n1
    ores = onumm9n1_create_r(0.0)

    # Set the coefficient to 1.
    onumm9n1_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm9n1.create(&ores)

  else:
    
    fres = feonumm9n1(0.0, nip)
    feonumm9n1_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm9n1
    ores = onumm9n1_create_r(1.0)
    res = onumm9n1.create(&ores)

  else:
    
    fres = feonumm9n1(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm9n1
    ores = onumm9n1_create_r(num)
    res = onumm9n1.create(&ores)

  else:
    
    fres = feonumm9n1(num, nip)
    res = fres
...
cpdef eye(uint64_t size, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm9n1.
    Ores = oarrm9n1_eye(size)

    return omatm9n1.create(&Ores)

  else:

    # This will create a feomatm9n1
    Fres = feoarrm9n1_eye(size,nip)

    return feomatm9n1.create(&Fres)
...
cpdef array( object arr,  uint64_t nip = 0):
  ...
  cdef oarrm9n1_t   Oarr
  cdef feoarrm9n1_t Farr
  cdef omatm9n1     Ores
  cdef feomatm9n1   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm9n1) or (tin is feomatm9n1):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm9n1_zeros( nrows, ncols)
        Ores = omatm9n1.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm9n1_zeros( nrows, ncols, nip)
        Fres = feomatm9n1.create(&Farr)
...
cpdef zeros( object shape_in, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm9n1.
    Ores = oarrm9n1_zeros(nrows,ncols)
    ...
    # This will create a feomatm9n1
    Fres = feoarrm9n1_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm9n1.
    Ores = oarrm9n1_ones(nrows,ncols)
    ...
    # This will create a feomatm9n1
    Fres = feoarrm9n1_ones(nrows,ncols,nip)
           # feoarrm9n1_ones(nrows,ncols,nip,nbases,order)
