...
cdef class csr_matrix:
    ...
    if targ1 == lil_matrix :
      ...
      for i in range(self.nrows):
        ...
        for j in range(len(lil.rows[i])):
          ...
          mdnum8_copy_to( &onum.num, &self.arr.p_data[k])
      ...
      if len(arg1) == 3: # Same as in scipy csr sparse.
        ...
        for i in range(nnz):
          ...
          if type(data[i]) == mdnum8:

            onum = data[i]
            mdnum8_copy_to( &onum.num, &self.arr.p_data[i])

          else:
            
            self.arr.p_data[i] = mdnum8_create_r( data[i] )
  ...
  def data(self): 
    ...
    return mdmat8.create(&self.arr, FLAGS=0)
  ...
  def order(self): 
    ...
    order = mdarr8_get_order(&self.arr)
  ...
  def copy(self):
    ...
    res.arr       = mdarr8_zeros( self.nnz, 1)
    ...
    mdarr8_copy_to( &self.arr, &res.arr)
  ...
  cpdef get_deriv(self, object hum_dir ):
    ...
    mdarr8_get_deriv_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef get_im(self, object hum_dir ):
    ...
    mdarr8_get_im_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef extract_im(self, object hum_dir):
    ...
    mdarr8_extract_im_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef  extract_deriv(self, object hum_dir):
    ...
    mdarr8_extract_deriv_to( indx, order, &self.arr, &res.arr)
  ...
  cpdef  get_order_im(self, ord_t order):
    ...
    mdarr8_get_order_im_to( order, &self.arr, &res.arr)
  ...
  cpdef  truncate( self, object hum_dir):
    ...
    mdarr8_truncate_im_to( indx, order, &self.arr, &res.arr)
  ...
  def get_active_bases(self):
    ...
    tmp = mdmat8.create(&self.arr,FLAGS=0)
...
# ****************************************************************************************************
cdef void csrmatrix_matmul_SO_to(csr_matrix lhs, mdmat8 rhs, mdmat8 res):
  """
  ...
  for i in range(lhs.nrows):
    ...
    for j in range(rhs.ncols):
      ...
      for l in range( lhs.p_indptr[i], lhs.p_indptr[i+1] ):
        ...
        mdnum8_gem_oo_to( &lhs.arr.p_data[l],
                        &rhs.arr.p_data[ j + k * rhs.ncols ],
                        &tmp, &tmp)

      # end for

      mdarr8_set_item_ij_o( &tmp, i, j, &res.arr)
...
# ****************************************************************************************************
cdef mdmat8 csrmatrix_matmul_SO(csr_matrix lhs, mdmat8 rhs):
  """
  PORPUSE:  Perform matrix-matrix multiplication between csr matrix and dense matrix.
...
# ****************************************************************************************************
cdef void csrmatrix_trunc_matmul_SO_to(ord_t ord_lhs, csr_matrix lhs, ord_t ord_rhs, mdmat8 rhs, mdmat8 res):
  """
  PORPUSE:  Perform matrix-matrix multiplication between csr matrix and dense matrix.
  ...
  for i in range(lhs.nrows):
    ...
    for j in range(rhs.ncols):
      ...
      for l in range( lhs.p_indptr[i], lhs.p_indptr[i+1] ):
        ...
        mdnum8_trunc_gem_oo_to( ord_lhs, &lhs.arr.p_data[l], 
...
# ****************************************************************************************************
cdef mdmat8 csrmatrix_trunc_matmul_SO(ord_t ord_lhs, csr_matrix lhs, ord_t ord_rhs, mdmat8 rhs):
  """
  PORPUSE:  Perform matrix-matrix multiplication between csr matrix and dense matrix.
