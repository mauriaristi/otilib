...
CONTAINS
    ...
    SUBROUTINE pytorch_graph_eval_m2n2(graph, stack_size, c_arr, c_size,x_matso,w_matso) BIND(C)
        ! Test to pass a graph as a rank-2 int64 array from C.
        IMPLICIT NONE

        ! subroutine inputs.
        INTEGER, INTENT(IN) :: stack_size, c_size  !< Array dimensions.
        TYPE(oarrm2n2_t), INTENT(IN) :: x_matso      !< input arrays. (Contains dimensions)
        TYPE(oarrm2n2_t), INTENT(INOUT) :: w_matso   !< output arrays. (Contains dimensions)
        ...
        TYPE(onumm2n2), POINTER :: x_arr(:,:)
        TYPE(onumm2n2), POINTER :: w_arr(:,:)

        TYPE(onumm2n2), POINTER :: p1_ptr(:) ! %%%%%% added new pointer variable.
        TYPE(onumm2n2), POINTER :: p2_ptr(:) ! %%%%%% added new pointer variable.
    ...
    END SUBROUTINE pytorch_graph_eval_m2n2

    SUBROUTINE sub_oo_vv_to(lhs,rhs,res)
      IMPLICIT NONE
      TYPE(onumm2n2), INTENT(IN) :: lhs(:)
      TYPE(onumm2n2), INTENT(IN) :: rhs(SIZE(lhs,1))
      TYPE(onumm2n2), INTENT(INOUT) :: res(SIZE(lhs,1))
   ...
   SUBROUTINE sin_v_to(X,RES)
      ...
      TYPE(onumm2n2), INTENT(INOUT) :: RES(SIZE(x,1))
   ...
   SUBROUTINE pow_oo_vv_to(lhs,rhs,res)
      ...
      DO i=1,SIZE(lhs,1)
          ! Order 2
          RES(i)%E11 = (1/2)*exp(RHS(i)%R*log(LHS(i)%R))*(2*(LHS(i)%E11*RHS(i)%R + LHS(i)%e1*RHS(i)%e1 + LHS(i)%e1*RHS(i)%e1*RHS(i)%R*log(LHS(i)%R))*LHS(i)%R + LHS(i)%e1**2*(-1 + RHS(i)%R)*RHS(i)%R + (2*RHS(i)%E11 + RHS(i)%e1**2*log(LHS(i)%R))*LHS(i)%R**2*log(LHS(i)%R))/LHS(i)%R**2
          ...
          RES(i)%E22 = (1/2)*exp(RHS(i)%R*log(LHS(i)%R))*(2*(LHS(i)%E22*RHS(i)%R + LHS(i)%e2*RHS(i)%e2 + LHS(i)%e2*RHS(i)%e2*RHS(i)%R*log(LHS(i)%R))*LHS(i)%R + LHS(i)%e2**2*(-1 + RHS(i)%R)*RHS(i)%R + (2*RHS(i)%E22 + RHS(i)%e2**2*log(LHS(i)%R))*LHS(i)%R**2*log(LHS(i)%R))/LHS(i)%R**2
          ! Order 1
...
END MODULE array_interface_m2n2