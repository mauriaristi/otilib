...
cpdef e( object hum_dir, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm1n7 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm1n7
    ores = onumm1n7_create_r(0.0)

    # Set the coefficient to 1.
    onumm1n7_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm1n7.create(&ores)

  else:
    
    fres = feonumm1n7(0.0, nip)
    feonumm1n7_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n7
    ores = onumm1n7_create_r(1.0)
    res = onumm1n7.create(&ores)

  else:
    
    fres = feonumm1n7(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm1n7
    ores = onumm1n7_create_r(num)
    res = onumm1n7.create(&ores)

  else:
    
    fres = feonumm1n7(num, nip)
    res = fres
...
cpdef eye(uint64_t size, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n7.
    Ores = oarrm1n7_eye(size)

    return omatm1n7.create(&Ores)

  else:

    # This will create a feomatm1n7
    Fres = feoarrm1n7_eye(size,nip)

    return feomatm1n7.create(&Fres)
...
cpdef array( object arr,  uint64_t nip = 0):
  ...
  cdef oarrm1n7_t   Oarr
  cdef feoarrm1n7_t Farr
  cdef omatm1n7     Ores
  cdef feomatm1n7   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm1n7) or (tin is feomatm1n7):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm1n7_zeros( nrows, ncols)
        Ores = omatm1n7.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm1n7_zeros( nrows, ncols, nip)
        Fres = feomatm1n7.create(&Farr)
...
cpdef zeros( object shape_in, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n7.
    Ores = oarrm1n7_zeros(nrows,ncols)
    ...
    # This will create a feomatm1n7
    Fres = feoarrm1n7_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm1n7.
    Ores = oarrm1n7_ones(nrows,ncols)
    ...
    # This will create a feomatm1n7
    Fres = feoarrm1n7_ones(nrows,ncols,nip)
           # feoarrm1n7_ones(nrows,ncols,nip,nbases,order)
