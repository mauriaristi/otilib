...
cpdef dot(object lhs, object rhs, object out = None):
  ...
  cdef mdmat10      Olhs, Orhs, Ores
  cdef mdarr10_t   cOres
  cdef dmat       Rlhs, Rrhs, Rres
  cdef darr_t    cRres
  cdef mdmatfe10    Flhs, Frhs, Fres
  ...
  if   tlhs is mdmatfe10:
    ...
    if   trhs is mdmatfe10: # FF
      ...
      if res_flag:
        Fres = out
        femdarr10_matmul_FF_to( &Flhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr10_matmul_FF( &Flhs.arr, &Frhs.arr )
      ...
      if res_flag:
        Fres = out
        femdarr10_matmul_FO_to( &Flhs.arr, &Orhs.arr ,&Fres.arr)
      else:
        cFres = femdarr10_matmul_FO( &Flhs.arr, &Orhs.arr )
        res = mdmatfe10.create(&cFres)
      # end if 
      ...
      if res_flag:
        Fres = out
        femdarr10_matmul_FR_to( &Flhs.arr, &Rrhs.arr ,&Fres.arr)
      else:
        cFres = femdarr10_matmul_FR( &Flhs.arr, &Rrhs.arr )
        res = mdmatfe10.create(&cFres)
      # end if 
    ...
    if   trhs is mdmatfe10: # OF
      ...
      if res_flag:
        Fres = out
        femdarr10_matmul_OF_to( &Olhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr10_matmul_OF( &Olhs.arr, &Frhs.arr )
        res = mdmatfe10.create(&cFres)
      # end if 
      ...
      if res_flag:
        Ores = out
        mdarr10_matmul_OO_to( &Olhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = mdarr10_matmul_OO( &Olhs.arr, &Orhs.arr )
      ...
      if res_flag:
        Ores = out
        mdarr10_matmul_OR_to( &Olhs.arr, &Rrhs.arr ,&Ores.arr)
      else:
        cOres = mdarr10_matmul_OR( &Olhs.arr, &Rrhs.arr )
        res = mdmat10.create(&cOres)
      # end if 
    ...
    if   trhs is mdmatfe10: # RF
      ...
      if res_flag:
        Fres = out
        femdarr10_matmul_RF_to( &Rlhs.arr, &Frhs.arr ,&Fres.arr)
      else:
        cFres = femdarr10_matmul_RF( &Rlhs.arr, &Frhs.arr )
        res = mdmatfe10.create(&cFres)
      # end if 
      ...
      if res_flag:
        Ores = out
        mdarr10_matmul_RO_to( &Rlhs.arr, &Orhs.arr ,&Ores.arr)
      else:
        cOres = mdarr10_matmul_RO( &Rlhs.arr, &Orhs.arr )
        res = mdmat10.create(&cOres)
      # end if 
...
cpdef transpose(object arr, object out = None):
  ...
  if   tarr is mdmatfe10:
    
    F = arr
    if res_flag:
      Fres = out
      femdarr10_transpose_to( &F.arr, &Fres.arr)
    else:
      cFres = femdarr10_transpose( &F.arr)
      res = mdmatfe10.create(&cFres)
    # end if 

  elif tarr is mdmat10:

    O = arr
    if res_flag:
      Ores = out
      mdarr10_transpose_to( &O.arr, &Ores.arr)
    else:
      cOres = mdarr10_transpose( &O.arr)
      res = mdmat10.create(&cOres)
    # end if    
...
cpdef det(object arr, object out = None):
  ...
  if   tarr is mdmatfe10:
    ...
    if res_flag:
      fres = out
      femdarr10_det_to( &F.arr, &fres.num)
    else:
      cfres = femdarr10_det( &F.arr)
    ...
    if res_flag:      
      ores = out
      mdarr10_det_to( &O.arr, &ores.num)
    else:
      cores = mdarr10_det( &O.arr)
...
cpdef norm(object arr, coeff_t p = 2.0, object out = None):
  ...
  if   tarr is mdmatfe10:
    ...
    if res_flag:
      fres = out
      femdarr10_pnorm_to( &F.arr, p, &fres.num)
    else:
      cfres = femdarr10_pnorm( &F.arr, p)
      res = mdnumfe10.create(&cfres)
    # end if 
    ...
    if res_flag:
      ores = out
      mdarr10_pnorm_to( &O.arr, p, &ores.num)
    else:
      cores = mdarr10_pnorm( &O.arr, p)
      res = mdnum10.create(&cores)
    # end if    
...
cpdef inv(object arr, object out = None):
  ...
  if   tarr is mdmatfe10:    
    ...
    if res_flag:
      Fres = out
      femdarr10_invert_to( &F.arr, &Fres.arr)
    else:
      cFres = femdarr10_invert( &F.arr)
      res = mdmatfe10.create(&cFres)
    # end if 
  elif tarr is mdmat10:
    O = arr
    if res_flag:
      Ores = out
      mdarr10_invert_to( &O.arr, &Ores.arr)
    else:
      cOres = mdarr10_invert( &O.arr)
      res = mdmat10.create(&cOres)
    # end if
  elif tarr is dmat:    
    R = arr
  ...
  if   tarr is mdmat10:    
    ...
    for i in range(Ores.arr.nrows):
      for j in range(Ores.arr.ncols):
        ...
        mdarr10_set_item_ij_r( inverse[i,j], i, j, &Ores.arr)
      # end for
    # end for    
...
#*****************************************************************************************************
cpdef solve(object K_in, mdmat10 b_in, mdmat10 out = None, solver = 'SuperLU',solver_args = {}):
  """
...
#*****************************************************************************************************
cdef solve_dense(mdmat10 K_in, mdmat10 b_in, mdmat10 out = None, solver = 'SuperLU',solver_args = {}):
  """
  ...
  for i in range(Ores.nrows):      
    for j in range(Ores.ncols):
      ...
      mdarr10_set_item_ij_r( rhs[i,j], i, j, &Ores.arr)
...
#*****************************************************************************************************
cpdef solve_sparse_old(csr_matrix K_in, mdmat10 b_in, mdmat10 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cdef solve_sparse(csr_matrix K_in, mdmat10 b_in, mdmat10 out = None, solver = 'SuperLU', solver_args = {}):
  """
  ...
  cdef mdmat10      O, Ores, Otmp, tmp, tmp2, tmp3
...
#*****************************************************************************************************
def solve_sparse_out(csr_matrix K_in, mdmat10 b_in, mdmat10 out = None):
  """
  PURPOSE:   Solve OTI linear system of equations for a dense K_in.
  """
  #***************************************************************************************************
...
#*****************************************************************************************************
cpdef get_order_im_array(ord_t ordi, mdmat10 tmp):
  """
  ...
  maxidx = mdnum10_get_ndir_order(ordi,&tmp.arr.p_data[0])
  order  = mdnum10_get_order( &tmp.arr.p_data[0] )
    ...
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        for k in range( maxidx ): 
          ...
          idx = mdnum10_get_indx(k, ordi)
          res[ i, j + tmp.ncols * k ] = mdnum10_get_item( idx, ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array(ord_t ordi, np.ndarray arr, mdmat10 tmp):
  """
  ...
  for i in range(tmp.nrows):
    ...
    for j in range(tmp.ncols):
      ...
      for k in range( nnz ):          
        ...
        mdnum10_set_item( val, idx, ordi, &otmp)

      # end for

      tmp[i,j] += mdnum10.create( &otmp)
...
#*****************************************************************************************************
cpdef get_order_im_array_2(ord_t ordi, mdmat10 tmp):
  """
  PURPOSE:   Get a specific order from .
  """
  #***************************************************************************************************
  ...
  for i in range(tmp.nrows):
    for j in range(tmp.ncols):
      ...
      if order >= ordi:
        ...
        coeff_list = mdnum10_get_order_address( ordi, &otmp)
...
#*****************************************************************************************************
cpdef set_order_im_from_array_2(ord_t ordi, np.ndarray arr, mdmat10 tmp):
  """
  PURPOSE:   Set a specific order from an array.
  """
  #***************************************************************************************************
