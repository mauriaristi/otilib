...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm2n7 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm2n7
    ores = onumm2n7_create_r(0.0)

    # Set the coefficient to 1.
    onumm2n7_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm2n7.create(&ores)

  else:
    
    fres = feonumm2n7(0.0, nip)
    feonumm2n7_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm2n7
    ores = onumm2n7_create_r(1.0)
    res = onumm2n7.create(&ores)

  else:
    
    fres = feonumm2n7(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm2n7
    ores = onumm2n7_create_r(num)
    res = onumm2n7.create(&ores)

  else:
    
    fres = feonumm2n7(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm2n7.
    Ores = oarrm2n7_eye(size)

    return omatm2n7.create(&Ores)

  else:

    # This will create a feomatm2n7
    Fres = feoarrm2n7_eye(size,nip)

    return feomatm2n7.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  cdef oarrm2n7_t   Oarr
  cdef feoarrm2n7_t Farr
  cdef omatm2n7     Ores
  cdef feomatm2n7   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm2n7) or (tin is feomatm2n7):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm2n7_zeros( nrows, ncols)
        Ores = omatm2n7.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm2n7_zeros( nrows, ncols, nip)
        Fres = feomatm2n7.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm2n7.
    Ores = oarrm2n7_zeros(nrows,ncols)
    ...
    # This will create a feomatm2n7
    Fres = feoarrm2n7_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm2n7.
    Ores = oarrm2n7_ones(nrows,ncols)
    ...
    # This will create a feomatm2n7
    Fres = feoarrm2n7_ones(nrows,ncols,nip)
           # feoarrm2n7_ones(nrows,ncols,nip,nbases,order)
