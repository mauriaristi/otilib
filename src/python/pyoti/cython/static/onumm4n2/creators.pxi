...
cpdef e( object hum_dir, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  """
  PURPOSE:  To create a onumm4n2 with value 1 at the specified imaginary direction
  ...
  if nip == 0: 
    ...
    # Create a onumm4n2
    ores = onumm4n2_create_r(0.0)

    # Set the coefficient to 1.
    onumm4n2_set_item(1.0, indx_hd, order_hd, &ores) 
    res = onumm4n2.create(&ores)

  else:
    
    fres = feonumm4n2(0.0, nip)
    feonumm4n2_set_im_r( 1.0, indx_hd, order_hd, &fres.num)
...
cpdef one( bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm4n2
    ores = onumm4n2_create_r(1.0)
    res = onumm4n2.create(&ores)

  else:
    
    fres = feonumm4n2(1.0, nip)
    res = fres
...
cpdef number( coeff_t num, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0: 
    ...
    # Create a onumm4n2
    ores = onumm4n2_create_r(num)
    res = onumm4n2.create(&ores)

  else:
    
    fres = feonumm4n2(num, nip)
    res = fres
...
cpdef eye(uint64_t size, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm4n2.
    Ores = oarrm4n2_eye(size)

    return omatm4n2.create(&Ores)

  else:

    # This will create a feomatm4n2
    Fres = feoarrm4n2_eye(size,nip)

    return feomatm4n2.create(&Fres)
...
cpdef array( object arr,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  cdef oarrm4n2_t   Oarr
  cdef feoarrm4n2_t Farr
  cdef omatm4n2     Ores
  cdef feomatm4n2   Fres
  cdef object res
  cdef np.ndarray np_arr

  tin = type(array)
  
  if (tin is omatm4n2) or (tin is feomatm4n2):
    ...
    if   np_arr.ndim == 0:
      ...
      if nip == 0:
        ...
        Oarr = oarrm4n2_zeros( nrows, ncols)
        Ores = omatm4n2.create(&Oarr)
        Ores[0,0] = np_arr.item(0)
        res = Ores

      else:
 
        Farr = feoarrm4n2_zeros( nrows, ncols, nip)
        Fres = feomatm4n2.create(&Farr)
...
cpdef zeros( object shape_in, bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm4n2.
    Ores = oarrm4n2_zeros(nrows,ncols)
    ...
    # This will create a feomatm4n2
    Fres = feoarrm4n2_zeros(nrows,ncols,nip)
...
cpdef ones( object shape_in,  bases_t nbases = 0, ord_t order = 0, uint64_t nip = 0):
  ...
  if nip == 0:
    ...
    # This will create an omatm4n2.
    Ores = oarrm4n2_ones(nrows,ncols)
    ...
    # This will create a feomatm4n2
    Fres = feoarrm4n2_ones(nrows,ncols,nip)
           # feoarrm4n2_ones(nrows,ncols,nip,nbases,order)
