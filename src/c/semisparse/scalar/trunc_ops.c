
// ****************************************************************************************************
void ssoti_trunc_smul_real(coeff_t a, ord_t ord, semiotin_t* res, dhelpl_t dhl){
    
    ord_t ordi;
    ndir_t i;

    if (a == 0.0){
        
        for (  ordi = ord-1; ordi < res->trc_order; ordi++){

            res->p_nnz[ordi]=0.0;

        }

        res->act_order = MIN(res->act_order,ord); // Update actual order

    } else {

        for (  ordi = ord-1; ordi < res->act_order; ordi++){

            for ( i=0; i<res->p_nnz[ordi]; i++){
                
                res->p_im[ordi][i] *= a;

            }

        }

    }    

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void ssoti_trunc_ssum(semiotin_t* num1, ord_t ord, semiotin_t* res, dhelpl_t dhl ){
    
    // Initialize
    ord_t ordi;

    // TODO: Add check to the resulting value.
    semiotin_t tmpres = ssoti_get_rtmp(6,res->trc_order,dhl); 
    
    ssoti_set_o(res, &tmpres, dhl);

    // Check this. What happens if one or the other is greater?
    tmpres.act_order = MAX(tmpres.act_order, num1->act_order);
    
    for (  ordi = ord-1; ordi < num1->act_order; ordi++){

        dhelp_sparse_add_dirs(num1->p_im[ordi],  num1->p_idx[ordi],   num1->p_nnz[ordi],
                               res->p_im[ordi],   res->p_idx[ordi],    res->p_nnz[ordi],
                             tmpres.p_im[ordi], tmpres.p_idx[ordi], &tmpres.p_nnz[ordi], dhl);

    }

    ssoti_set_trunc( &tmpres, ord, res, dhl);
    
}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void ssoti_trunc_mul(semiotin_t* num1, ord_t ord1, 
                    semiotin_t* num2, ord_t ord2, 
                    semiotin_t* dest, dhelpl_t dhl ){

    semiotin_t tmp, tmp2, tmp3;
    semiotin_t* tmpsrc=  &tmp ;
    semiotin_t* tmpdest= &tmp3;
    semiotin_t* tmpswap;
    ord_t max_ord = MAX(num1->trc_order,num2->trc_order);
    ord_t res_ord;
    ord_t ordlim1, ordlim2;
    ord_t ordi1, ordi2, ordires;

    // Retreive semiotin temporals.
    // All tmps are created with no elements in imaginary directions (but allocated).
    tmp = ssoti_get_rtmp(3,max_ord,dhl); // will hold the final result.
    tmp2= ssoti_get_rtmp(4,max_ord,dhl); // Will hold the temporary result.
    tmp3= ssoti_get_rtmp(5,max_ord,dhl); // Will hold the temporary result.    
    
    res_ord = MIN(num1->act_order + num2->act_order, max_ord );
    tmp.act_order = res_ord;
    tmp2.act_order= res_ord;
    tmp3.act_order= res_ord;


    ordlim1 = MIN(num1->act_order, max_ord - 1);
    
    for(ordi1=ord1-1; ordi1<ordlim1; ordi1++){

        ordlim2 = MIN(num2->act_order, max_ord - (ordi1+1) );

        tmpswap = tmpsrc; tmpsrc=tmpdest; tmpdest=tmpswap;

        // printf("Lim ord2: %hhu\n",ordlim2);
        for (ordi2=ord2-1; ordi2<ordlim2; ordi2++){

            ordires = ordi1 + ordi2 + 1;

            dhelp_sparse_mult(num1->p_im[ordi1  ], num1->p_idx[ordi1  ], num1->p_nnz[ordi1  ], ordi1+1,
                              num2->p_im[ordi2  ], num2->p_idx[ordi2  ], num2->p_nnz[ordi2  ], ordi2+1,
                               tmp2.p_im[ordires],  tmp2.p_idx[ordires], &tmp2.p_nnz[ordires],     dhl );
            
            dhelp_sparse_add_dirs(    tmp2.p_im[ordires],     tmp2.p_idx[ordires],      tmp2.p_nnz[ordires],
                                   tmpsrc->p_im[ordires],  tmpsrc->p_idx[ordires],   tmpsrc->p_nnz[ordires],
                                  tmpdest->p_im[ordires], tmpdest->p_idx[ordires], &tmpdest->p_nnz[ordires], dhl);
            
        }

        ordires = ordi1+ord2;

        tmpsrc->p_im[ ordires] = tmpdest->p_im[ ordires];
        tmpsrc->p_idx[ordires] = tmpdest->p_idx[ordires];
        tmpsrc->p_nnz[ordires] = tmpdest->p_nnz[ordires];

    }
    

    ssoti_set_trunc( tmpdest, ord1+ord2-1, dest, dhl);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
void ssoti_set_trunc( semiotin_t* src, ord_t ord, semiotin_t* dest, dhelpl_t dhl){
    
    // Assumes both have the space allocated and both have the same truncation order
    ord_t i;
    
    dest->act_order = MAX(src->act_order,dest->act_order);

    for ( i = ord-1; i<src->act_order; i++){
        
        // Set number of non-zero and allocated size to 0.
        dest->p_nnz[i]  = src->p_nnz[i];

        memcpy(dest->p_im[i], src->p_im[i], dest->p_nnz[i]*sizeof(coeff_t));
        memcpy(dest->p_idx[i],src->p_idx[i],dest->p_nnz[i]*sizeof(imdir_t));

    }

}
// ----------------------------------------------------------------------------------------------------