...
void mdarr7_taylor_integrate_to( coeff_t* deltas, mdarr7_t* arr, mdarr7_t* res){
    ...
    mdarr7_dimCheck_OO_elementwise(arr,res,res);

    for (i = 0; i<arr->size; i++ ){
        
        mdnum7_taylor_integrate_to( deltas, &arr->p_data[i], &res->p_data[i]);
...
mdarr7_t mdarr7_taylor_integrate( coeff_t* deltas, mdarr7_t* arr){
    ...
    mdarr7_taylor_integrate_to( deltas, arr, &res);
...
void mdarr7_get_order_im_to( ord_t order, mdarr7_t* arr, mdarr7_t* res){
    ...
    for (i = 0; i<arr->size; i++ ){
        ...
        mdnum7_get_order_im_to( order, &arr->p_data[i], &res->p_data[i]);
...
mdarr7_t mdarr7_get_order_im( ord_t order, mdarr7_t* arr){
    ...
    mdarr7_get_order_im_to( order, arr, &res);
...
void mdarr7_truncate_im_to( imdir_t idx, ord_t order, mdarr7_t* arr, mdarr7_t* res){
    ...
    for (i = 0; i<arr->size; i++ ){
        ...
        mdnum7_truncate_im_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
mdarr7_t mdarr7_truncate_im( imdir_t idx, ord_t order, mdarr7_t* arr){
    ...
    mdarr7_truncate_im_to( idx, order, arr, &res);
...
void mdarr7_copy_to(mdarr7_t* arr, mdarr7_t* res){
    ...
    for (i=0; i<arr->size; i++){
        ...
        mdnum7_copy_to(&arr->p_data[i],&res->p_data[i]);
...
// ****************************************************************************************************
inline void mdarr7_set_all_r( coeff_t num, mdarr7_t* arr){
    
    uint64_t i;

    for (i = 0; i<arr->size; i++ ){
        mdarr7_set_item_i_r( num, i, arr);
    }
...
// ****************************************************************************************************
inline void mdarr7_set_r( coeff_t num, mdarr7_t* arr){
...
// ****************************************************************************************************
inline void mdarr7_set_item_i_r( coeff_t num, uint64_t i, mdarr7_t* arr){
        
    if( i<arr->size ){
        
        mdnum7_set_r( num, &arr->p_data[i]);

    } else {

        // res = mdnum7_init();
        
        printf("ERROR: Index out of bounds in mdarr7_get_item_i(...)\n Exiting...\n");
        exit(OTI_BadIndx);
...
// ****************************************************************************************************
inline void mdarr7_set_item_ij_r( coeff_t num, uint64_t i, uint64_t j, mdarr7_t* arr){

    mdarr7_set_item_i_r( num, j + i*arr->ncols, arr);

}
// ----------------------------------------------------------------------------------------------------

// ****************************************************************************************************
inline void mdarr7_set_all_o( mdnum7_t* num, mdarr7_t* arr){

    uint64_t i;

    for (i = 0; i<arr->size; i++ ){
        mdarr7_set_item_i_o( num, i, arr);
    }
...
// ****************************************************************************************************
inline void mdarr7_set_o( mdnum7_t* num, mdarr7_t* arr){
...
// ****************************************************************************************************
inline void mdarr7_set_item_i_o( mdnum7_t* src, uint64_t i, mdarr7_t* arr){
    
    if (i < arr->size){

        mdnum7_copy_to(src, &arr->p_data[i]);
...
// ****************************************************************************************************
inline void mdarr7_set_item_ij_o( mdnum7_t* num, uint64_t i, uint64_t j, mdarr7_t* arr){

    if ( i < arr->nrows && j < arr->ncols ){

        mdarr7_set_item_i_o( num, j + i*arr->ncols, arr);

    } else {

        printf("ERROR: Index out of bounds in mdarr7_get_item_ij(...)\n Exiting...\n");
        exit(OTI_BadIndx);
...
// ****************************************************************************************************
void mdarr7_set_slice_O( mdarr7_t* arr, 
    ...
    for ( i = 0; i < nrows; i++){
        ...
        for ( j = 0; j < ncols; j++){
            ...
            mdnum7_copy_to( &arr->p_data[  j +  i * arr->ncols ], 
...
// ****************************************************************************************************
void mdarr7_set_slice_o( mdnum7_t* num, 
                        int64_t starti, int64_t stopi, int64_t stepi,
                        int64_t startj, int64_t stopj, int64_t stepj,
                         mdarr7_t* res){
...
darr_t mdarr7_get_im(imdir_t idx, ord_t order, mdarr7_t* arr){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        darr_set_item_i(  
            mdnum7_get_item( idx, order, &arr->p_data[i]), 
            i, &res);
...
mdarr7_t mdarr7_extract_im(imdir_t idx, ord_t order, mdarr7_t* arr){
    ...
    mdarr7_t res = mdarr7_zeros( arr->nrows, arr->ncols);
    
    mdarr7_extract_im_to( idx, order, arr, &res);
...
void mdarr7_extract_im_to(imdir_t idx, ord_t order, mdarr7_t* arr, mdarr7_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum7_extract_im_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
darr_t mdarr7_get_deriv(imdir_t idx, ord_t order, mdarr7_t* arr){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        darr_set_item_i(  
            mdnum7_get_deriv( idx, order, &arr->p_data[i]), 
            i, &res);
...
mdarr7_t mdarr7_get_im_o(imdir_t idx, ord_t order, mdarr7_t* arr){
    ...
    mdarr7_get_im_to( idx, order, arr, &res);
...
void mdarr7_get_im_to(imdir_t idx, ord_t order, mdarr7_t* arr, mdarr7_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum7_get_im_to_o( idx, order, &arr->p_data[i], &res->p_data[i]);
...
mdarr7_t mdarr7_get_deriv_o(imdir_t idx, ord_t order, mdarr7_t* arr){
    ...
    mdarr7_get_deriv_to( idx, order, arr, &res);
...
void mdarr7_get_deriv_to(imdir_t idx, ord_t order, mdarr7_t* arr, mdarr7_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum7_get_deriv_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
mdarr7_t mdarr7_extract_deriv(imdir_t idx, ord_t order, mdarr7_t* arr){
    ...
    mdarr7_extract_deriv_to( idx, order, arr, &res);
...
void mdarr7_extract_deriv_to(imdir_t idx, ord_t order, mdarr7_t* arr, mdarr7_t* res){
    ...
    for( i = 0; i < arr->size ; i++ ){
        ...
        mdnum7_extract_deriv_to( idx, order, &arr->p_data[i], &res->p_data[i]);
...
// Getters.
// ****************************************************************************************************
inline mdnum7_t mdarr7_get_item_i(mdarr7_t* arr, uint64_t i){
...
mdnum7_t mdarr7_get_item_ij(mdarr7_t* arr, uint64_t i, uint64_t j){
    ...
    mdnum7_t res = mdarr7_get_item_i(arr, j + i*arr->ncols );
...
void mdarr7_get_item_i_to(mdarr7_t* arr, uint64_t i, mdnum7_t* res){
    ...
    if (i < arr->size){
        ...
        mdnum7_copy_to( &arr->p_data[i], res);
        
    } else {
        // Raise error?
        printf("ERROR: Index out of bounds in mdarr7_get_item_i_to(...)\n");
        exit(OTI_BadIndx);
    }
...
void mdarr7_get_item_ij_to(mdarr7_t* arr, uint64_t i, uint64_t j, mdnum7_t* res){
    ...
    mdarr7_get_item_i_to(arr, j + i*arr->ncols, res);
...
mdarr7_t mdarr7_get_slice( mdarr7_t* arr, 
    ...
    res = mdarr7_zeros( nrows, ncols);

    mdarr7_get_slice_to( arr, starti, stopi, stepi, startj, stopj, stepj, &res);
...
void mdarr7_get_slice_to( mdarr7_t* arr, 
    ...
    for ( i = 0; i < nrows; i++){
        ...
        for ( j = 0; j < ncols; j++){
            ...
            mdnum7_copy_to( &arr->p_data[ jj + ii * arr->ncols ], 
...
mdarr7_t mdarr7_ones(uint64_t nrows, uint64_t ncols){
    ...
    res = mdarr7_createEmpty(nrows, ncols);

    for (i=0; i<res.size; i++ ){
        mdnum7_set_r(1.0,&res.p_data[i]);
    }
...
// ****************************************************************************************************
inline mdarr7_t mdarr7_empty_like(mdarr7_t* arr){

    mdarr7_t  res;
    
    res = mdarr7_createEmpty(arr->nrows, arr->ncols);    
...
// ****************************************************************************************************
inline mdarr7_t mdarr7_createEmpty(uint64_t nrows, uint64_t ncols){

    mdarr7_t  res ;
    uint64_t size = nrows * ncols;

    res.p_data = (mdnum7_t*) malloc( size * sizeof( mdnum7_t ) );

    if ( res.p_data == NULL ){
        printf("ERROR: Not enough memory to handle mdarr7.\n Exiting...\n");
...
// Initialization
// ****************************************************************************************************
inline mdarr7_t mdarr7_init(void){
