...
def ode_step(F,y0,t0,eps=1e-3,order = 5, dt_tol = 1e-11):
    ...
    INPUTS:
        - F: Callable function evaluating F( t, y)The function that will be integrated.
        - x0: The starting point for integration.
        - t0: The initial t related with x0.
        - eps: the tolerance to find a time step.
        - order: Order that will be used for OTI integration.
        - dt_tol: Tolerance for 

    OUTPUTS:
        [x, dt]

        -  x: OTI array (matso) with information about the result.
        - dt: Maximum estimated timestep from this integration with order..
    """
    
    if x0 is oti.matso : # Array type.
        x = oti.zeros(x0.shape)
    else:
        x = oti.zeros((1,1))
    # end if
    
    t = t0 + oti.e(1, order = order) # create the corresponding perturbation.

    for i in range(order+1):
        
        x_t = F( t, x) # TODO: What if extra arguments are needed?
